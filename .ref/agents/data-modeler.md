...
5. `.kiro/steering/ulid-prefix-registry.md` — prefixed-ULID conventions.
6. The owning service's `api/openapi.yaml` — cross-service schemas that ERD row
   shapes must match.
...
- Row-attribution compliance review against new Supabase schemas.
...
- Row-attribution compliance verified clean on the design.
