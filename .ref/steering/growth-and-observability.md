---
inclusion: fileMatch
fileMatchPattern: "**/growth/**|**/observability/**|**/analytics.json|**/marketing.json"
---

# Growth and observability lanes

The vocabulary lock-in for every "event fired somewhere" concern in the Stackra
backend. Five distinct lanes exist. They share primitives (events, providers,
MultipleInstanceManager, per-tenant configs, consent gates) but answer different
questions, serve different audiences, and have different failure semantics.
Confusing them is the #1 source of subtle bugs in cross-cutting concerns — an
event captured under the wrong lane gets the wrong retention, wrong consent
gate, wrong delivery guarantee, and wrong audit trail.
...
| **Audit**      | `observability.audit`      | Who did what, when? (compliance-grade)           | Compliance, DPO, regulators | Medium                         | Zero                                           | 7 years min.              | Audit-log library; module blueprint deferred   |
| **Activity**   | `observability.activity`   | What happened recently in this tenant? (UX feed) | Tenant users                | Medium                         | Low                                            | 90d–1yr per tier          | Activity-log library; module blueprint deferred |
...
## 4. The shared shape — the driver manager

Every lane that fans out to multiple providers uses the workspace's canonical
multi-instance driver manager (`@stackra/support` on the frontend; the
equivalent driver-manager utility on the backend).

```
LaneManager (extends the driver manager)
    → instance('provider_name') → LaneProvider driver
...
interface LaneProvider {
  name(): string;
  supports(type: LaneEventType): boolean;
  dispatch(event: LaneEvent, config: LaneProviderConfig): Promise<DeliveryResult>;
  transform(event: LaneEvent): Record<string, unknown>; // provider-native payload
}
```
...
Same pattern as `events.json` (declarations) → the event dispatcher
...
- **Migration review**: new tables that ship any of `event_type`, `provider`,
  ...
  comment + wire through the appropriate module's driver manager.
- **Code review**: domain code never dispatches a marketing or analytics event
  class directly. Domain services fire domain events; lane-specific listeners
  route them.
- **Consent audit**: every `<lane>_events` row must carry a non-null
  ...
  forgot the gate).
- **Retention audit**: a purge-expired-events job per lane runs nightly. Every
  lane has its own retention config (`<lane>.retention`).
...
- `observability-signals.md` — audit / activity / monitoring lanes.
- `growth-signals.md` — attribution / marketing / analytics / referrals lanes.
