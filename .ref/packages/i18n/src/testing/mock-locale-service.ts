...
import type { II18nLocaleService, ILocaleItem } from "@stackra/contracts";
...
export class MockLocaleService implements II18nLocaleService {
  ...
  private readonly localeItems: readonly ILocaleItem[];
  ...
  public constructor(options?: {
    ...
    locales?: readonly ILocaleItem[];
  }) {
    this.currentLocale = options?.defaultLocale ?? "en";
    this.supportedLocales = options?.supportedLocales ?? [this.currentLocale];

    const supplied = new Map<string, ILocaleItem>();
    for (const item of options?.locales ?? []) supplied.set(item.code, item);
  ...
  public getLocales(): readonly ILocaleItem[] {
