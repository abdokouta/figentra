 ...
 * @description Barrel export for react-subpath interfaces (component
 *   props + resolver options). Cross-platform hook return types
 *   (`UseI18nReturn`, `UseLocaleReturn`, `UseDirectionReturn`,
 *   `IUseFormatReturn`) live in `../../core/interfaces` and are
 *   re-exported here so consumers can
 *   `import { UseI18nReturn } from '@stackra/i18n/react'`.
 */

// ── Component props ────────────────────────────────────────────────────
export type { LanguageComboboxProps } from "./language-combobox-props.interface";
