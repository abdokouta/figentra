...
} from "../utils/resolve-language.util";
import {
...
export class I18nManager {
  ...
  public constructor(@Inject(I18N_CONFIG) config: II18nConfig) {
    // ADR-0063 — module binds options AS-IS. Apply defaults inline
    // by merging over `DEFAULT_I18N_CONFIG` at construction time.
    this.config = { ...DEFAULT_I18N_CONFIG, ...config };
    const defaultLocale = this.config.defaultLocale;
    this.supportedLocales =
      this.config.supportedLocales && this.config.supportedLocales.length > 0
        ? this.config.supportedLocales
        : [defaultLocale];
    this.getCurrentLocale = () => defaultLocale;
  ...
  public translate(key: string, options?: TranslateOptions): string {
    ...
    if (resolvedLang !== this.config.defaultLocale) {
      const nextFallback = getNextFallbackLanguage(
        resolvedLang,
  ...
  public async loadNamespace(
    ...
    if (!this.translations[currentLocale])
      this.translations[currentLocale] = {};
    (this.translations[currentLocale] as Record<string, unknown>)[namespace] =
      translations;
  }
  ...
  private resolveKey(
    ...
    const count =
      options?.count ??
      (args && !Array.isArray(args) ? (args as any).count : undefined);
    if (count !== undefined) {
    ...
    if (typeof value === "object" && value !== null) {
      const shouldReturn =
        options?.returnObjects ?? this.config.returnObjects ?? true;
      if (!shouldReturn) return key;
      if (Array.isArray(value)) {
        const join = options?.joinArrays ?? this.config.joinArrays;
        if (typeof join === "string")
          return (value as unknown[]).map(String).join(join);
      }
    ...
    if (typeof value === "string" && args) {
      const prefix =
        this.config.interpolation?.prefix ?? DEFAULT_INTERPOLATION_PREFIX;
      const suffix =
        this.config.interpolation?.suffix ?? DEFAULT_INTERPOLATION_SUFFIX;
