...
export interface UseI18nReturn<K = Record<string, unknown>> {
  ...
  languages: string[];
  /**
   * Display metadata for every supported locale (name + flag).
   * Derived from `II18nConfig.locales` at boot; every entry falls
   * back to `{ code, name: code }` when the app didn't supply
   * display data. Locale pickers (`LanguageSelector`,
   * `LanguageToggle`, custom zone adapters) read this instead of
   * hardcoding a display map at the call site.
   */
