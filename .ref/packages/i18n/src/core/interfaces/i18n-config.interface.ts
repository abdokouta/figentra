...
export interface II18nConfig {
  ...
  /**
   * Display metadata for each supported locale — the name + flag
   * pickers render next to the BCP-47 code. When omitted, the
   * runtime derives an item per `supportedLocales` entry with the
   * code itself as the name and no flag — hardcoded per-app
   * display maps in zone adapters are no longer needed.
   *
   * Length + order do NOT need to match `supportedLocales`
   * exactly — items whose code isn't in `supportedLocales` are
   * silently skipped by the manager; supported codes with no
   * entry fall back to the derived `{ code, name: code }` shape.
   *
   * Typical shape:
   *
   * ```ts
