...
/**
 * Key-form overload for schema-only settings groups — `T` widens to
 * `Record<string, unknown>` since there is no compile-time schema.
 *
 * @param groupKey - Group key registered via
 *   `SettingsModule.forFeature({...})`.
 * @param options - Optional mutation-mode + rollback + undoable knobs.
 */
...
/**
 * Runtime dispatch signature for the two overloads above. Consumers
 * type-check against the overloads; this body picks between DTO or
 * key form based on the runtime `typeof dtoOrKey`.
 */
export function useUpdateSettings<T extends object>(
      ...
      try {
        if (mode === "pessimistic") {
          ...
          await runUndoable({
            ...
            dtoOrKey,
            partial,
            undoableQueue: undoableQueue ?? undefined,
            undoableTimeout,
...
/**
 * Shared mutation context passed to every mode implementation.
 *
 * @typeParam T - Settings-group value shape.
 */
export interface IModeContext<T> {
  /** The settings service resolving writes for the target group. */
  readonly service: ISettingsService;
  /** Group key resolved from the DTO or supplied by the caller. */
  readonly groupKey: string;
  /** Original DTO / key argument (retained for `getByKey` fallbacks). */
  readonly dtoOrKey: Type<T> | string;
  /** Values the caller wants to apply. */
  readonly partial: Partial<T>;
  /** Snapshots the pre-mutation values before applying `partial`. */
  readonly applyWithSnapshot: (partial: Partial<T>) => Record<string, unknown>;
  /** Restore the pre-mutation snapshot (called on failure / cancel). */
...
/**
 * Optimistic-mode context — adds the event bus so the mode can
 * subscribe to `SETTINGS_EVENTS.UPDATE_FAILED` for automatic
 * rollback.
 *
 * @typeParam T - Settings-group value shape.
 */
export interface IOptimisticContext<T> extends IModeContext<T> {
  /** Event bus for the rollback subscription. `undefined` disables the auto-rollback. */
...
/**
 * Undoable-mode context — extends {@link IModeContext} with the
 * undoable queue + timeout + label + cancel-hook required for the
 * "Gmail-style undo" flow.
 *
 * @typeParam T - Settings-group value shape.
 */
export interface IUndoableContext<T> extends IModeContext<T> {
  /** Undoable queue from `@stackra/query`. `undefined` degrades to fire-and-forget. */
  readonly undoableQueue?: IUndoableQueue;
  /** Millis until the queued mutation auto-commits if no cancel arrives. */
  readonly undoableTimeout: number;
  /** Optional label shown in the undo toast. */
  readonly undoableLabel?: string;
  /** Invoked with a `cancel` fn so the caller can wire up a toast. */
