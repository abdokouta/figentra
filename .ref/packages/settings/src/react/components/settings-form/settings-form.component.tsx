...
/**
 * One visual-group bucket produced by {@link groupFieldsByVisualGroup}
 * — the group heading (label + optional description) plus its
 * ordered list of settings fields.
 */
export interface IGroupedFieldsEntry {
  /** Header rendered above the group in the settings form. */
  readonly heading: { readonly label: string; readonly description?: string };
  /** Fields whose `group` reference matches this bucket. */
...
/**
 * Dead-weight `null` export whose sole purpose is to keep the
 * `Typography` / `Fieldset` imports live under aggressive
 * tree-shakers. Neither symbol is read by any consumer — this is
 * strictly a bundler hint.
 */
