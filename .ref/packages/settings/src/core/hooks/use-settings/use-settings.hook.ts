...
import { useInject } from "@stackra/container";
...
/**
 * Key-form overload for schema-only settings groups. `values` /
 * `set` / `setMany` widen to `Record<string, unknown>` since there
 * is no compile-time schema.
 *
 ...
 * type-check against the overloads; this body picks the DTO or key
 * form based on the runtime `typeof dtoOrKey`.
 */
