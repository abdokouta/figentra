...
/**
 * Key-form overload for schema-only settings groups — degrades to
 * `unknown` return since there is no compile-time schema to narrow.
 *
 * @param groupKey - Group key registered via
 *   `SettingsModule.forFeature({...})`.
 * @param fieldKey - Field key inside the group.
 */
 ...
 * type-check against the overloads; this body routes to `useSettings`
 * under either form.
 */
