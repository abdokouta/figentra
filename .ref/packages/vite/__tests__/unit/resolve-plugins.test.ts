...
import { PluginResolutionError } from "../../src/core/errors/plugin-resolution.error";
import { ViteConfigError } from "../../src/core/errors/vite-config.error";
import type { IPluginMap } from "../../src/core/interfaces/plugin-map.interface";
import { resolvePlugins } from "../../src/core/utils/resolve-plugins.util";
...
describe("resolvePlugins", () => {
  ...
  it("wraps a factory throwing a non-Error value into a PluginResolutionError with an Error cause", async () => {
    ...
    try {
      ...
      expect((err as PluginResolutionError).cause?.message).toBe(
        "plain string",
      );
    }
  });
