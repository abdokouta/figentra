...
async function demo() {
  ...
  (output as any).link(
    "GitHub Repository",
    "https://github.com/stackra/monorepo",
  );
  (output as any).link(
    "Issue Tracker",
    "https://github.com/stackra/monorepo/issues",
  );
  output.newLine();
  ...
  if (process.stdout.isTTY && process.argv.includes("--interactive")) {
    ...
    const framework = await output.select("Pick a framework:", [
      {
        value: "@stackra/container",
        label: "@stackra/container",
        hint: "recommended",
      },
    ...
    (output as any).separator(60, "Interactive Prompts");
    output.info(
      "Skipped interactive prompts (non-TTY or --interactive flag not passed)",
    );
