  ...
  public async handle(): Promise<number> {
    ...
    if (all) {
      ...
      out.table(
        ...
        packages.map((p) => [
          p,
          `packages/platform/${p}/config/${p}.config.ts`,
          "✓ published",
        ]),
      );
      ...
      this.output.success(
        `Published config for "${pkg}"${force ? " (forced)" : ""}.`,
      );
    }
    return 0;
  }
...
class ConfigCacheCommand extends BaseCommand {
  ...
  public async handle(): Promise<number> {
    ...
    this.output.success(
      "Configuration cache written to config/cache/config.json",
    );
    return 0;
  }
}
...
@Command({
  ...
  options: [
    { name: "--path", short: "-p", description: "Custom glob", type: "string" },
  ],
})
@Injectable()
...
async function main() {
  const app = await ApplicationFactory.createApplicationContext(
    FullTestCliModule,
    {
      logger: ["error"],
    },
