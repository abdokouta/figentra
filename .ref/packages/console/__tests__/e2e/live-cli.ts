...
@Command({
  name: "config:publish",
  description:
    "Publish a package config file to the application config directory",
  arguments: [
    { name: "package", description: "Package name", required: false },
  ],
  options: [
    {
      name: "--force",
...
@Command({
  name: "config:cache",
  description: "Show config files available for caching",
})
@Injectable()
...
class ConfigClearCommand extends BaseCommand {
  ...
  public async handle(): Promise<void> {
    ...
    this.output.success(
      `Cache exists at ${cachePath} (${size} KB) — ready to clear.`,
    );
  }
}
...
class ModuleListCommand extends BaseCommand {
  ...
  public async handle(): Promise<void> {
    ...
    const filtered = typeFilter
      ? manifests.filter((m) => m.type === typeFilter)
      : manifests;

    if (filtered.length === 0) {
      this.output.info(
        typeFilter
          ? `No modules with type "${typeFilter}".`
          : "No manifests found.",
      );
      return;
    }
...
@Command({
  ...
  description: "Discover all module manifests in the workspace",
})
@Injectable()
class ModuleDiscoverCommand extends BaseCommand {
  ...
  public async handle(): Promise<number> {
    ...
    this.output.newLine();
    this.output.success(
      `Workspace scan complete — ${manifests.length} modules discovered.`,
    );
    return 0;
  }
...
@Command({
  ...
  arguments: [
    { name: "path", description: "Path to manifest.json", required: true },
  ],
})
@Injectable()
...
async function main() {
  ...
  if (!commandName || commandName === "list") {
    ...
    (output as any).table(
      ...
      registry
        .getAll()
        .map((c) => [c.namespace || "general", c.name, c.description]),
    );
    ...
    if (!entry) {
      ...
      if (suggestions.length > 0)
        (output as any).info(`Did you mean: ${suggestions.join(", ")}?`);
      await app.close();
      process.exit(1);
    }
