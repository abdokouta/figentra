...
import { DuplicateCommandError } from "../../src/errors";
import { CommandRegistry } from "../../src/registries";

import type { IRegisteredCommand } from "../../src/interfaces";
...
describe("CommandRegistry", () => {
  ...
  function createEntry(
    overrides: Partial<IRegisteredCommand> = {},
  ): IRegisteredCommand {
    return {
      name: "test:command",
  ...
  describe("register()", () => {
    ...
    it("should associate subcommands with parent", () => {
      registry.register(
        createEntry({ name: "queue", namespace: "", subcommands: [] }),
      );
      registry.register(
        createEntry({
          name: "queue:work",
  ...
  describe("getByNamespace()", () => {
    it("should filter commands by namespace", () => {
      registry.register(
        createEntry({ name: "config:publish", namespace: "config" }),
      );
      registry.register(
        createEntry({ name: "config:cache", namespace: "config" }),
      );
      registry.register(
        createEntry({ name: "queue:work", namespace: "queue" }),
      );
  ...
  describe("getNamespaces()", () => {
    it("should return all unique non-empty namespaces sorted", () => {
      ...
      registry.register(
        createEntry({ name: "cache:clear", namespace: "cache" }),
      );
