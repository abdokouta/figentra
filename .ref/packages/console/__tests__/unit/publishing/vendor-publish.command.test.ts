...
import { VendorPublishCommand } from "../../../src/commands/vendor-publish.command";
import { PublishableRegistry } from "../../../src/publishing/registries/publishable.registry";
import { StubRenderer } from "../../../src/services/stub-renderer.service";
...
describe("VendorPublishCommand", () => {
  ...
  describe("empty registry", () => {
    it("prints an info message and exits 0 when no publishables are registered", async () => {
      ...
      const message =
        (output.info.mock.calls[0]?.[0] as string | undefined) ?? "";
  ...
  describe("--tag flag", () => {
    ...
    it("returns exit code 1 and reports the error when the tag is unknown", async () => {
      ...
      const errorMessage =
        (output.error.mock.calls[0]?.[0] as string | undefined) ?? "";
    ...
    it("emits ONE success line naming the count when the write succeeds", async () => {
      ...
      const successMessage =
        (output.success.mock.calls[0]?.[0] as string | undefined) ?? "";
  ...
  describe("--all flag", () => {
    it("publishes every registered entry", async () => {
      ...
      expect(exitCode).toBe(0);
      expect(existsSync(path.join(sandbox.cwd, "config/a.config.ts"))).toBe(
        true,
      );
      expect(existsSync(path.join(sandbox.cwd, "config/b.config.ts"))).toBe(
        true,
      );
    });
  });
  ...
  describe("--dry-run flag", () => {
    it("does not write any files when --dry-run is passed", async () => {
      ...
      const infoMessage =
        (output.info.mock.calls.at(-1)?.[0] as string | undefined) ?? "";
      ...
      registry.register(
        {
          ...
          files: [
            {
              from: "config/does-not-exist.ts",
              to: "config/does-not-exist.ts",
            },
          ],
        },
        SourceModule,
  ...
  describe("interactive multiselect (no --tag / --all)", () => {
    it("uses the interactive picker to select entries", async () => {
      ...
      const options = output.multiselect.mock.calls[0]?.[1] as {
        value: string;
      }[];
    ...
    it("bails without side effects when the operator selects nothing", async () => {
      ...
      const warningMessage =
        (output.warning.mock.calls[0]?.[0] as string | undefined) ?? "";
  ...
  describe("atomic write — no half-written destinations left behind", () => {
    it("removes the temp file after a successful write", async () => {
      ...
      const tmpFiles = siblings.filter(
        (f) => f.endsWith(".tmp") || f.includes(".tmp-"),
      );
