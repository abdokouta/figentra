...
import { PublishableLoader } from "../../../src/publishing/services/publishable-loader.service";
...
describe("PublishableLoader", () => {
  ...
  describe("consumer construction", () => {
    ...
    it("attributes registered entries to the correct source module", () => {
      ...
      expect(registry.byTag("cfg-a")?.sourceModule).toBe(
        ModuleWithPublishables,
      );
      expect(registry.byTag("multi-a")?.sourceModule).toBe(
        ModuleWithMultiplePublishables,
      );
      expect(registry.byTag("multi-b")?.sourceModule).toBe(
        ModuleWithMultiplePublishables,
      );
    });
  ...
  describe("this-binding", () => {
    it("preserves `this` inside `configurePublishables` so sibling statics still resolve", () => {
      ...
      expect(ModuleThatInspectsThis.seenThis.value).toBe(
        ModuleThatInspectsThis,
      );
    });
  });
  ...
  describe("fail-loud on bad manifest", () => {
    ...
    it("propagation includes duplicate errors when two modules claim the same tag", () => {
      ...
      const loader = new PublishableLoader(discovery, registry);
      expect(() => loader.onApplicationBootstrap()).toThrow(
        /already registered/,
      );
    });
  });
