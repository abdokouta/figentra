...
import { PublishableConsumer } from "../../../src/publishing/publishable.consumer";
import { PublishableRegistry } from "../../../src/publishing/registries/publishable.registry";

import type {
  IPublishableEntry,
  IPublishableRegistryEntry,
  Type,
...
describe("PublishableConsumer", () => {
  ...
  describe("packageRoot auto-fill", () => {
    it("reads `sourceModule.PACKAGE_ROOT` when the entry omits `packageRoot`", () => {
      const consumer = new PublishableConsumer(
        registry,
        ModuleWithRoot as Type<unknown>,
      );
      ...
      expect(registry.byTag("cfg-a")?.packageRoot).toBe(
        "/abs/module-with-root",
      );
    });
    ...
    it("explicit entry.packageRoot wins over the class static", () => {
      ...
      expect(registry.byTag("cfg-explicit")?.packageRoot).toBe(
        "/abs/explicit-override",
      );
    });
    ...
    it("falls through to empty string when class has no PACKAGE_ROOT (registry then rejects with a helpful message)", () => {
      const consumer = new PublishableConsumer(
        registry,
        ModuleWithoutRoot as Type<unknown>,
      );
  ...
  describe("files[] normalization — object shorthand", () => {
    ...
    it("preserves the `render` flag when explicitly set", () => {
      ...
      consumer.publish({
        ...
        files: [
          {
            from: "stubs/processor.txt",
            to: "stubs/processor.txt",
            render: true,
          },
        ],
      });
    ...
    it("preserves `render: false` (opt-out of `.ejs` template rendering)", () => {
      ...
      consumer.publish({
        ...
        files: [
          {
            from: "stubs/verbatim.ejs",
            to: "stubs/verbatim.ejs",
            render: false,
          },
        ],
      });
  ...
  describe("files[] normalization — mixed forms", () => {
    it("normalizes a mixed string + object files array", () => {
      ...
      expect(stored?.files).toEqual([
        ...
        { from: "stubs/processor.ejs", to: "app/stubs/processor.ejs" },
        {
          from: "stubs/scheduler.ejs",
          to: "stubs/scheduler.ejs",
          render: false,
        },
      ]);
    });
  ...
  describe("sourceModule attribution — passed to registry", () => {
    ...
    it("each consumer pins a DIFFERENT sourceModule (no state bleed)", () => {
      ...
      const consumerA = new PublishableConsumer(
        registry,
        ModA as Type<unknown>,
      );
      const consumerB = new PublishableConsumer(
        registry,
        ModB as Type<unknown>,
      );
