...
import { DuplicatePublishableTagError } from "../../../src/publishing/errors/duplicate-tag.error";
import { InvalidPublishableEntryError } from "../../../src/publishing/errors/invalid-publishable-entry.error";
...
describe("PublishableRegistry", () => {
  ...
  describe("register — happy path", () => {
    ...
    it("preserves the description field", () => {
      registry.register(
        makeEntry({ description: "The routing config." }),
        TestModuleA,
      );
      expect(registry.byTag("test-config")?.description).toBe(
        "The routing config.",
      );
    });
  ...
  describe("register — tag validation", () => {
    it("rejects a non-string tag", () => {
      ...
      expect(() => {
        registry.register(
          makeEntry({ tag: 42 as unknown as string }),
          TestModuleA,
        );
    ...
    it("accepts a valid multi-hyphen tag", () => {
      ...
      expect(() => {
        registry.register(
          makeEntry({ tag: "stackra-ui-brand-assets" }),
          TestModuleA,
        );
      }).not.toThrow();
    });
  ...
  describe("register — packageRoot validation", () => {
    ...
    it("rejects a relative packageRoot", () => {
      ...
      const err = tryRegister(() =>
        registry.register(
          makeEntry({ packageRoot: "./relative/path" }),
          TestModuleA,
        ),
      );
    ...
    it("accepts an absolute POSIX packageRoot", () => {
      ...
      expect(() => {
        registry.register(
          makeEntry({ packageRoot: "/absolute/path" }),
          TestModuleA,
        );
      }).not.toThrow();
    });
  ...
  describe("register — files validation", () => {
    it("rejects a missing files array (typed as unknown to bypass compile-time check)", () => {
      const err = tryRegister(() =>
        registry.register(
          makeEntry({
            files: undefined as unknown as IPublishableRegistryEntry["files"],
          }),
          TestModuleA,
        ),
      );
    ...
    it("rejects an empty files array — publishable with zero files is meaningless", () => {
      const err = tryRegister(() =>
        registry.register(makeEntry({ files: [] }), TestModuleA),
      );
      expect(err).toBeInstanceOf(InvalidPublishableEntryError);
      expect(err.field).toBe("files");
  ...
  describe("register — (key, value) overload (BaseRegistry contract compat)", () => {
    ...
    it("validates through the same code path (rejects bad tags)", () => {
      ...
      expect(() => registry.register("bad_tag", value)).toThrow(
