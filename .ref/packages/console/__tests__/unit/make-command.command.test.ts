...
import { MakeCommandCommand } from "../../src/commands/make-command.command";
  ...
  describe("scaffolding — happy path", () => {
    it("renders the stub and writes the file", async () => {
      ...
      const dest = path.join(
        sandbox.cwd,
        "src/commands/cache-clear.command.ts",
      );
    ...
    it("emits success + pairs summary on the output", async () => {
      ...
      const pairsArg = output.pairs.mock.calls[0]?.[0] as Record<
        string,
        string
      >;
