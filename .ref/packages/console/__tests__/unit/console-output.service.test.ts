...
import { DEFAULT_ICONS } from "../../src/services/theme.service";
...
describe("ConsoleOutput", () => {
  ...
  describe("interactive methods in non-TTY", () => {
    ...
    it("should throw CommandCancelledError for select() in non-TTY", async () => {
      await expect(
        output.select("Choose:", [{ value: "a", label: "A" }]),
      ).rejects.toThrow("Interactive prompts are not available");
    });

    it("should throw CommandCancelledError for multiselect() in non-TTY", async () => {
      await expect(
        output.multiselect("Choose:", [{ value: "a", label: "A" }]),
      ).rejects.toThrow("Interactive prompts are not available");
    });
  });
