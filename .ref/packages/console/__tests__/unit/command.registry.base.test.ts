...
import { ConsoleError, DuplicateCommandError } from "../../src/errors";
...
describe("CommandRegistry — BaseRegistry adoption", () => {
  ...
  describe("method overload — register(entry) vs register(key, value)", () => {
    ...
    it("returns `this` from both overloads for chaining", () => {
      ...
      const returned2 = registry.register(
        "cmd-b",
        createEntry({ name: "cmd-b" }),
      );
  ...
  describe("makeDuplicateError override", () => {
    ...
    it("`DuplicateCommandError` extends `ConsoleError` (family kinship for catch-all consumers)", () => {
      registry.register(
        createEntry({ name: "dup", classRef: class ExistingClass {} }),
      );
      let caught: unknown = null;
      try {
        registry.register(
          createEntry({ name: "dup", classRef: class SecondClass {} }),
        );
      } catch (err) {
        caught = err;
      }
    ...
    it("names BOTH class refs in the duplicate error", () => {
      ...
      try {
        registry.register(
          createEntry({ name: "dup", classRef: SecondCommand }),
        );
      } catch (err) {
