...
export class ConsoleModule {
   ...
   *
   * Register `ConsoleModule` exactly once in the root CLI module.
   * The module is `@Global` and its providers (registries, loader,
   * output) hold process-level state; calling `forRoot()` a second
   * time in the same process is a wiring bug that should be caught
   * during app-shell review, not at runtime. Follows the convention
   * every other `forRoot(...)` in the workspace uses — no bespoke
   * flag, no runtime double-init lock. Tests build fresh containers
   * per case via `@stackra/testing`, so `forRoot()` is called once
   * per container regardless of test count.
   *
   * @param options - Module configuration (all fields optional, defaults applied)
   * @returns Global dynamic module definition
   */
  public static forRoot(
    options: Partial<IConsoleModuleOptions> = {},
  ): DynamicModule {
    const resolvedConfig: Required<IConsoleModuleOptions> = {
      binaryName: options.binaryName ?? DEFAULT_BINARY_NAME,
      commandsDirectory:
        options.commandsDirectory ?? DEFAULT_COMMANDS_DIRECTORY,
   ...
   *
   * Register `ConsoleModule` exactly once in the root CLI module —
   * see the note on {@link ConsoleModule.forRoot} for why there's no
   * runtime double-init lock. Same rules apply here.
   *
   * @param options - Async module configuration with factory provider
   * @returns Global dynamic module definition
   */
  public static forRootAsync(
    options: IConsoleModuleAsyncOptions,
  ): DynamicModule {
    return {
      module: ConsoleModule,
      global: true,
        ...
        // its default so the factory can return a partial shape.
        {
          ...
          useFactory: async (...args: unknown[]) => {
            ...
            return {
              ...
              commandsDirectory:
                config.commandsDirectory ?? DEFAULT_COMMANDS_DIRECTORY,
