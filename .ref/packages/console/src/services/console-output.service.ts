...
export class ConsoleOutput implements IConsoleOutput {
  ...
  public pairs(pairs: Record<string, string> | [string, string][]): void {
    ...
    for (const [key, value] of entries) {
      // Route padding through `Str.padRight` per
      // `.kiro/steering/support-utilities.md`.
      this.writeln(
        `${palette.label(Str.padRight(key, maxKeyLen))}  ${palette.value(value)}`,
      );
    }
  }
  ...
  public separator(width = 60, label?: string): void {
    ...
    if (label) {
      ...
      this.writeln(
        palette.border(`${leftPad} ${palette.label(label)} ${rightPad}`),
      );
    } else {
  ...
  public json(data: unknown, label?: string): void {
    ...
    const highlighted = formatted
      .replace(
        /"([^"]+)":/g,
        (_: string, key: string) => `${palette.primary(`"${key}"`)}:`,
      )
      .replace(
        /: "([^"]+)"/g,
        (_: string, val: string) => `: ${palette.success(`"${val}"`)}`,
      )
      .replace(
        /: (\d+)/g,
        (_: string, num: string) => `: ${palette.warning(num)}`,
      )
      .replace(
        /: (true|false)/g,
        (_: string, bool: string) => `: ${palette.info(bool)}`,
      )
  ...
  public list(
    items: string[],
    options?: { style?: "bullet" | "pointer" | "numbered" },
  ): void {
    const { icons, palette } = this.theme;
    ...
    items.forEach((item, index) => {
      ...
      switch (style) {
        case "pointer":
          ...
          break;
        case "numbered":
          // Route padding through `Str.padLeft` per
          // `.kiro/steering/support-utilities.md`.
          prefix = palette.muted(`${Str.padLeft((index + 1).toString(), 2)}.`);
          break;
        default:
  ...
  public table(headers: string[], rows: string[][]): void {
    ...
    const table = new Table({
      head: this.isInteractive
        ? headers.map((h) => palette.accent(h))
        : headers,
      style: { head: [], border: [] },
      chars: {
        top: "─",
  ...
  public async tasks(items: ITaskItem[]): Promise<void> {
    ...
    for (const item of items) {
      ...
      try {
        ...
        s.stop(
          `${item.title} ${this.theme.palette.error(`— ${errorMessage}`)}`,
          1,
        );
        throw error;
      }
