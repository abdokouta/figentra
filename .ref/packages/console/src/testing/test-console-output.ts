...
export class TestConsoleOutput implements IConsoleOutput {
  ...
  private assertMethodCalled(method: string, message?: string): void {
    ...
    if (methodCalls.length === 0) {
      throw new Error(
        `Expected "${method}" to be called but it was never invoked.`,
      );
    }

    if (message) {
      const found = methodCalls.some((c) =>
        String(c.args[0]).includes(message),
      );
      if (!found) {
