...
@Command({
  ...
  arguments: [
    {
      name: "namespace",
      description: "Filter commands by namespace",
      required: false,
    },
  ],
})
export class ListCommand extends BaseCommand {
  ...
  public constructor(
    ...
    @Inject(CONSOLE_CONFIG)
    private readonly config: Required<IConsoleModuleOptions>,
  ) {
    super();
  }
  ...
  private renderAll(): void {
    ...
    process.stdout.write(
      `  ${pc.dim(`${total} command(s) in ${nsCount} category(ies)`)}\n`,
    );
    process.stdout.write(
  ...
  private renderCommandEntry(
    ...
    // `.kiro/steering/support-utilities.md` — canonical helper, never
    // a bare `.padEnd(...)`.
    const paddedName = Str.padRight(name, 30);
    const sourceTag = source ? pc.dim(` [${source}]`) : "";
    process.stdout.write(
      `    ${pc.green(paddedName)} ${pc.dim(description)}${sourceTag}\n`,
    );
  }
}
