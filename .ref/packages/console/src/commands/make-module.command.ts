...
@Command({
  name: "make:module",
  description:
    "Scaffold a new @Module-decorated class with forRoot + configurePublishables wired.",
  arguments: [
    {
      name: "name",
...
export class MakeModuleCommand extends BaseCommand {
  ...
  public async handle(): Promise<number> {
    ...
    if (!description) {
      description = await this.output.text(
        "Enter a one-line description for the module:",
        {
    ...
    if (existsSync(filePath) && !force) {
      const overwrite = await this.output.confirm(
    ...
    this.output.info(
      `Import ${className}.forRoot() into your AppModule to activate it.`,
    );
    return 0;
  }
}
