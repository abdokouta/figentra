...
@Command({
  name: "vendor:publish",
  description:
    "Publish package resources (config, stubs, migrations, ...) to the app.",
  options: [
    {
      name: "--tag",
...
export class VendorPublishCommand extends BaseCommand {
  ...
  public async handle(): Promise<void | number> {
    ...
    if (tag) {
      ...
      const chosen = await this.output.multiselect(
        "Which resources should be published?",
        options,
      );
      const chosenSet = new Set(chosen);
      selected = this.registry
        .all()
        .filter((entry) => chosenSet.has(entry.tag));
  ...
  private finish(
    ...
    if (errored > 0) {
      this.output.error(
        `${written} written, ${skipped} skipped, ${errored} errored.`,
      );
      return 1;
    }
    if (flags.dryRun) {
