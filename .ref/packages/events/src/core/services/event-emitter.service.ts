...
import { EVENT_EMITTER_CONFIG_TOKEN } from '../constants';
 ...
 * or the `EVENT_EMITTER_TOKEN` symbol.
 *
 * @example
 * ```typescript
...
export class EventEmitter {
  ...
  public constructor(
    @Optional()
    @Inject(EVENT_EMITTER_CONFIG_TOKEN)
   ...
   *
   * Integrates with `@stackra/nestjs-orm`'s EntityManager flush lifecycle.
   *
  ...
  private addListener(
    ...
    if (this.maxListeners > 0 && entries.length > this.maxListeners) {
      console.warn(
        `[EventEmitter] Possible memory leak: ${entries.length} listeners for ` +
