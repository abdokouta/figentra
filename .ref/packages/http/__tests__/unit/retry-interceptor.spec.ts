...
import { RetryInterceptor } from "../../src/core/interceptors/retry.interceptor";
...
function totalDefaultWaits(): number {
  return DEFAULT_RETRY_BACKOFF.slice(0, DEFAULT_MAX_RETRIES).reduce(
    (a, b) => a + b,
    0,
  );
}
...
describe("RetryInterceptor", () => {
  ...
  describe("non-retryable failures", () => {
    it("re-throws immediately on 4xx without emitting a retry", async () => {
      ...
      await expect(interceptor.intercept(makeContext(), next)).rejects.toBe(
        err,
      );
      expect(next).toHaveBeenCalledTimes(1);
      expect(emitter.emitted).toHaveLength(0);
  ...
  describe("per-request overrides via `meta`", () => {
    ...
    it("honours `meta.retryBackoff` — custom per-attempt delays", async () => {
      ...
      await settled;
      expect(
        emitter.emitted.map((e) => (e.payload as { delayMs: number }).delayMs),
      ).toEqual(custom);
    });
  });
