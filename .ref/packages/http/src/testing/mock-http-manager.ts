...
export class MockHttpManager implements IHttpManager {
  ...
  private readonly middlewareRegistries = new Map<
    string,
    IHttpMiddlewareRegistry
  >();

  /** Per-connection interceptor registries. */
  private readonly interceptorRegistries = new Map<
    string,
    IHttpInterceptorRegistry
  >();
  ...
  public forgetConnection(name?: string | string[]): void {
    ...
    // is short-circuited above so `name` is `string | string[]` here.
    for (const target of Arr.wrap(name)) {
  ...
  public getConnectionNames(): string[] {
    return Array.from(
      new Set([...this.configs.keys(), ...this.connections.keys()]),
    );
  }
  ...
  public createClientFromConnector(
    ...
    const name =
      (config["__connectionName"] as string | undefined) ??
  ...
  public extend(
    _driver: string,
    _factory: (config: Record<string, unknown>) => IHttpClient,
  ): void {
