 ...
 * @description `@HttpMiddleware()` class decorator.
 *
...
export function HttpMiddleware(
  options: IHttpMiddlewareOptions = {},
): ClassDecorator {
 ...
 */
export function getHttpMiddlewareMetadata(
  target: Function,
): IHttpMiddlewareOptions | undefined {
  return getMetadata<IHttpMiddlewareOptions>(
    HTTP_MIDDLEWARE_METADATA,
    target as object,
  );
}
