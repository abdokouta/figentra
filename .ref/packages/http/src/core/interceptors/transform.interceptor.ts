 ...
 * @description Transform interceptor.
 *
...
export class TransformInterceptor implements IHttpInterceptor {
  ...
  public async intercept(
    ...
    const transformConfig =
      this.config.connections[this.config.default]?.transform;
    if (!transformConfig?.enabled) return next(context);
    if (context.request.meta?.["skipTransform"] === true) return next(context);

    if (
      context.request.data !== undefined &&
      transformConfig.requestTransform
    ) {
      try {
  ...
  private transformRequest(request: IHttpRequestConfig): unknown {
    ...
    const customRequest =
      transformConfig?.customTransforms?.[endpoint]?.request;
    if (customRequest) {
      try {
    ...
    if (transformConfig?.dateHandling?.serializeDates) {
      try {
        ...
        throw new HttpTransformError(
          `Date serialization failed: ${(err as Error).message}`,
          value,
        );
      }
    }
  ...
  private transformResponse(
    ...
    const customResponse =
      transformConfig?.customTransforms?.[endpoint]?.response;
    if (customResponse) {
      try {
    ...
    if (transformConfig?.dateHandling?.parseDates) {
      try {
        ...
        throw new HttpTransformError(
          `Date parsing failed: ${(err as Error).message}`,
          value,
        );
      }
    }
