 ...
 * @description Fetch connector — the framework's DEFAULT driver
 *   since 1.1.0.
 *
 ...
 *
 *   Registered automatically by `HttpModule.forRoot(...)` under the
 *   driver name `"fetch"` (see `http.module.ts`'s
 *   `BUILT_IN_CONNECTORS`). Consumer apps get a working HTTP layer
 *   out of the box without installing `axios` as a peer dep — the
 *   pre-1.1.0 workspace-wide pain point.
 *
 *   ```typescript
 *   // Zero-config — every connection uses FetchConnector by default.
 *   @Module({
 ...
 *   ```
 *
 *   Consumers who explicitly want the axios connector for a
 *   specific connection set `driver: "axios"` on that connection's
 *   config + install `axios` as a peer dep. See
 *   `./axios.connector.ts` for the reasons to prefer axios per-case.
 *
 *   The `@stackra/http/fetch` subpath continues to re-export this
 *   class for backward compatibility with pre-1.1.0 consumers.
 */
...
export class FetchConnector implements IHttpConnector {
  ...
  public async send(context: IHttpContext): Promise<IHttpResponse> {
    ...
    const data = await FetchConnector.decodeBody(
      response,
  ...
  private static buildInit(
    ...
    const init: RequestInit = {
      ...
      headers,
      signal: FetchConnector.combineTimeoutSignal(
        request.signal,
        request.timeout,
      ),
    };
   ...
   */
  private static async decodeBody(
    response: Response,
    responseType?: string,
  ): Promise<unknown> {
    switch (responseType) {
