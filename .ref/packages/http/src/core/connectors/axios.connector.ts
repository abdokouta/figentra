 ...
 * @description Axios connector.
 *
 *   Alternative driver — since 1.1.0 `FetchConnector` is the
 *   default (browser-native + zero-dep). Consumers who want axios
 *   opt in via `driver: "axios"` on the connection config AND
 *   install `axios` as a peer dep — this connector lazy-imports
 *   the module at first use and throws a friendly
 *   `[AxiosConnector] 'axios' is not installed` error otherwise.
 *
 *   Reasons to reach for axios over fetch:
 *   - Rich interceptor ecosystem (third-party plugins already
 *     wired against axios).
 *   - Native cancellation via `AbortController` + `CancelToken`.
 *   - Node-side streaming without the fetch API's Node ≥ 18 floor.
 *
 *   Wraps `axios.request()` for unary requests and
...
export class AxiosConnector implements IHttpConnector {
   ...
   */
  private static async *toAsyncIterable(
    data: unknown,
