...
export class HttpClient implements IHttpClient {
  ...
  /** @inheritdoc */
  public stream<T = unknown>(
    url: string,
    config?: IStreamConfig,
  ): IHttpStream<T> {
    const format = config?.format ?? HttpStreamFormat.Sse;
    return this.openStream<T>(
      { ...config, url, method: config?.method ?? "GET" },
      format,
    );
  }
  ...
  private async executeRequest<T>(
    perRequestConfig: IHttpRequestConfig,
  ): Promise<IHttpResponse<T>> {
    ...
    try {
      const response = (await this.middlewarePipeline.execute(
        ...
        context,
        (ctx) =>
          this.interceptorPipeline.execute(
            this.interceptorRegistry.getSorted(),
            ctx,
            (innerCtx) => this.connector.send(innerCtx),
          ),
      )) as IHttpResponse<T>;
  ...
  private openStream<T>(
    ...
    async function* iterator(): AsyncIterableIterator<T> {
      ...
      const sseParser =
        parser instanceof SseStreamParser
          ? (parser as SseStreamParser<unknown>)
          : null;
      ...
      try {
        ...
        throw err instanceof HttpStreamError
          ? err
          : new HttpStreamError(
              `[HttpClient:${self.name}] stream failed`,
              err as Error,
            );
      }
    }
