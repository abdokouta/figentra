  ...
  implements IHttpManager, OnModuleInit, OnModuleDestroy
{
  ...
  private readonly middlewareRegistries: Map<string, IHttpMiddlewareRegistry> =
    new Map();

  /** Connection name → interceptor registry. */
  private readonly interceptorRegistries: Map<
    string,
  ...
  public async onModuleInit(): Promise<void> {
    if (!this.config?.default) {
      this.logger.warn(
        "[HttpManager] config.default is missing — skipping warm-up.",
      );
      return;
    }
  ...
  /** @inheritdoc */
  public override getInstanceConfig(
    name: string,
    ...
    return {
      ...
      // Default driver flipped from `"axios"` → `"fetch"` in 1.1.0.
      // `fetch` is browser-native + zero-dep so every consumer app
      // works out of the box. Consumers who explicitly want axios
      // still opt in via `driver: "axios"` on the connection config.
      driver: raw.driver ?? "fetch",
  ...
  public createClientFromConnector(
    ...
    const connectionName =
      (config["__connectionName"] as string | undefined) ?? "default";
    ...
    const interceptorRegistry =
      this.interceptorRegistries.get(connectionName) ??
  ...
  public forgetConnection(name?: string | string[]): void {
    ...
    // Route the single-or-many normalisation through `Arr.wrap` per
    // `.kiro/steering/support-utilities.md` — the `undefined` case
    // is short-circuited above so `name` is `string | string[]` at
    // this point.
    const targets = Arr.wrap(name);
  ...
  /** @inheritdoc */
  public async getMiddlewareRegistry(
    name?: string,
  ): Promise<IHttpMiddlewareRegistry> {
  ...
  /** @inheritdoc */
  public async getInterceptorRegistry(
    name?: string,
  ): Promise<IHttpInterceptorRegistry> {
    const connectionName = name ?? this.config.default;
