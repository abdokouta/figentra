...
| `@stackra/container/worker`  | Cloudflare Worker bootstrap, runtime bindings, request scope |
...
Cloudflare Workers are supported through the thin `@stackra/container/worker` runtime adapter. Cloudflare remains responsible for the Worker runtime and Bindings; the container owns application composition and request scope.

```ts
import { WorkerFactory } from "@stackra/container/worker";
import { Injectable, Module, REQUEST_SCOPE, Inject } from "@stackra/container";
import { WORKER_REQUEST, WORKER_ENV, WORKER_EXECUTION_CONTEXT } from "@stackra/container/worker";
...
class Handler {
  ...
  handle(): Response {
    return Response.json({ url: this.request.url });
  }
}
...
export default WorkerFactory.create(AppModule, { handler: Handler });
```

Runtime values are supplied per request through `WORKER_ENV`, `WORKER_REQUEST`, `WORKER_EXECUTION_CONTEXT`, and `WORKER_CONTEXT`. Do not treat Cloudflare Bindings as a replacement for DI. Bind them at the infrastructure boundary and expose application-level abstractions to business services.
