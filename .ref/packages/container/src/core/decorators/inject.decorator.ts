  ...
  return (target: object, key: string | symbol | undefined, index?: number) => {
    ...
    if (!resolvedToken && !hasExplicitToken) {
      ...
      if (key !== undefined) {
        ...
        const paramTypes =
          getMetadata<InjectionToken[]>(PARAMTYPES_METADATA, target) ?? [];
    ...
    if (index !== undefined) {
      ...
      updateMetadata(
        ...
        (props) => [
          ...props,
          {
            key: key as string | symbol,
            type: resolvedToken as InjectionToken,
          },
        ],
