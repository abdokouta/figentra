 ...
 * @description DependenciesScanner — Module Tree Scanner
 *
...
import { DynamicModule, Provider } from "@stackra/contracts";
...
export class DependenciesScanner {
  ...
  private async reflectImports(
    ...
    const staticImports =
      getMetadata<any[]>(MODULE_METADATA.IMPORTS, metatype) ?? [];
  ...
  private reflectProviders(metatype: ModuleMetatype, token: string): void {
    const staticProviders =
      getMetadata<Provider[]>(MODULE_METADATA.PROVIDERS, metatype) ?? [];
  ...
  private reflectExports(metatype: ModuleMetatype, token: string): void {
    const staticExports =
      getMetadata<any[]>(MODULE_METADATA.EXPORTS, metatype) ?? [];
    ...
    for (const exported of allExports) {
      ...
      if (
        exportToken &&
        typeof exportToken === "function" &&
        !providerTokens.has(exportToken)
      ) {
        // Check if it's a module being re-exported (has MODULE_METADATA)
        const isModule =
          getMetadata<any[]>(MODULE_METADATA.IMPORTS, exportToken) !==
          undefined ||
          getMetadata<any[]>(MODULE_METADATA.PROVIDERS, exportToken) !==
          undefined ||
  ...
  private getExportToken(exported: any): any {
    ...
    if (typeof exported === "symbol" || typeof exported === "string")
      return exported;
    if (typeof exported === "object" && "provide" in exported)
      return exported.provide;
    if (typeof exported === "object" && "module" in exported)
