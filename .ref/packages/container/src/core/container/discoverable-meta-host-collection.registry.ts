 ...
 * @description DiscoverableMetaHostCollection — Static index of provider classes
 ...
 *
 * @support-utilities-exempt All-static class over WeakMap-scoped tables
 *   (class → metadata key, plus per-{@link ModuleContainer} reverse
 *   index). Not an instance-based registry — never instantiated. Also
 *   constrained by the same circular-dep concern as
 *   {@link ModuleContainer}: {@link BaseRegistry} lives in
 *   `@stackra/support` which depends on `@stackra/container`.
 */
export class DiscoverableMetaHostCollection {
  ...
  private static getMetaKeyByInstanceWrapper(
    ...
    if (instanceWrapper.metatype && !instanceWrapper.inject) {
      ...
      classRef =
        (instanceWrapper.instance as { constructor?: Function } | null)
          ?.constructor ?? (instanceWrapper.metatype as Function | null);
    }
