 ...
 * @description Injector — Dependency Resolution Engine
 *
...
import type { RequestContext } from "@/core/contexts/request";
...
export class Injector {
  ...
  private readonly requestResolutionStacks = new Map<string, Set<InjectionToken>>();

  private getResolutionStack(context?: RequestContext): Set<InjectionToken> {
    if (!context) return this.resolutionStack;
    let stack = this.requestResolutionStacks.get(context.id);
    if (!stack) {
      stack = new Set<InjectionToken>();
      this.requestResolutionStacks.set(context.id, stack);
    }
    return stack;
  }
  ...
  public async resolveProviders(moduleRef: Module): Promise<void> {
    ...
    for (const [_token, wrapper] of providers) {
      // Request-scoped providers are instantiated lazily for each request.
      if (!wrapper.isResolved && !wrapper.isRequestScoped) {
  ...
  public async resolveInstance<T>(
    ...
    // Already resolved application singleton — return cached instance.
    if (wrapper.isResolved && !wrapper.isTransient && !wrapper.isRequestScoped) {
      return wrapper.instance!;
    }

    // Request-scoped providers require a request context and are cached per
    // context rather than on the application-level wrapper.
    if (wrapper.isRequestScoped) {
      if (!requestContext) {
        throw new Error(
          `Provider '${wrapper.name}' is request-scoped and can only be resolved inside a request context.`,
        );
      }
      const cached = wrapper.getRequestInstance(requestContext.id);
      if (cached !== undefined) return cached;
    }

    // Circular dependency detection
    const resolutionStack = this.getResolutionStack(requestContext);

    if (resolutionStack.has(wrapper.token)) {
      throw new Error(
        `Circular dependency detected: ${this.formatResolutionStack(wrapper.token, requestContext)}`,
      );
    }

    resolutionStack.add(wrapper.token);
    ...
    try {
      ...
      if (wrapper.isFactory) {
        // ── Factory provider ─────────────────────────────────────────
        instance = await this.resolveFactory(wrapper, moduleRef, requestContext);
      } else if (wrapper.metatype) {
        // ── Class provider ───────────────────────────────────────────
        instance = await this.resolveClass(wrapper, moduleRef, requestContext);
      } else {
      ...
      if (wrapper.isRequestScoped) {
        wrapper.setRequestInstance(requestContext!.id, instance);
      } else {
      ...
      resolutionStack.delete(wrapper.token);
    }
  }
  ...
  public lookupProvider(
    ...
    // 1. Check own providers by exact identity.
    if (moduleRef.providers.has(token)) {
      return { wrapper: moduleRef.providers.get(token)!, host: moduleRef };
    }

    // 2. Check imported modules' exports by exact identity.
    const imported = this.lookupInImports(token, moduleRef, new Set());
    if (imported) return imported;

    // 3. Class-name fallback — resilient to bundle duplication where
    //    a class token gets multiple identities in the same runtime.
    //
    //    In a pnpm-based monorepo, a package's class (e.g.
    //    `StateRegistry`) can end up in two chunks — one bundled with
    //    the app's direct import path, another via a transitive
    //    resolution through a different peer-dep set. `useClass:
    //    StateRegistry` in `StateModule.forRoot()` captures reference
    //    A, while a downstream `forFeature` factory's decorator
    //    `design:paramtypes` may capture reference B. Both have the
    //    same `.name` but `A !== B`, so exact-identity `.has()` fails
    //    even though a functionally-equivalent provider exists.
    //
    //    This fallback ONLY applies to function tokens (classes). It
    //    NEVER matches symbol tokens — those are shared via
    //    `Symbol.for()` and any identity mismatch there is a real
    //    misconfiguration we want to surface loudly.
    //
    //    See `.kiro/backlog-frontend-2026-07-27.md` §"Class-token
    //    duplication" for context.
    if (typeof token === "function") {
      const tokenName = (token as { name?: string }).name;
      if (tokenName) {
        const byName = this.findClassByName(tokenName, moduleRef, new Set());
  ...
  /**
   * Fallback lookup by class name — used only when exact-identity
   * lookup fails for a class token.
   *
   * Rules (mirror `lookupInImports`'s visibility rules so this
   * fallback never widens visibility beyond what the module graph
   * intentionally exposes):
   *
   * 1. Search the CURRENT module's providers freely (own scope).
   * 2. Search each imported module's providers ONLY when the
   *    imported module ALSO lists a same-named export in its
   *    `exports` set — this preserves the "non-exported providers
   *    are not visible to consumers" invariant tested by
   *    `injector-resolution` and `module-imports`.
   *
   * @param name - The class name to match (`token.name`).
   * @param moduleRef - The starting module.
   * @param visited - Cycle-safety set.
   * @param isOwn - When true, search all providers (own scope).
   *   When false, only match providers whose token is also in the
   *   module's `exports` set (imported scope).
   * @returns The wrapper and host, or undefined when no matching
   *   same-named class provider is reachable.
   */
  private findClassByName(
    ...
    if (visited.has(moduleRef.id)) return undefined;
    visited.add(moduleRef.id);

    for (const [key, wrapper] of moduleRef.providers) {
      if (typeof key !== "function") continue;
      if ((key as { name?: string }).name !== name) continue;
      // Exported check applies only when we're crossing a module
      // boundary. The starting module (own scope) can match any of
      // its providers, exported or not.
      if (!isOwn && !moduleRef.exports.has(key)) continue;
      return { wrapper, host: moduleRef };
    }

    for (const imported of moduleRef.imports) {
      const found = this.findClassByName(name, imported, visited, false);
  ...
  private async resolveClass<T>(
    ...
    for (let index = 0; index < deps.length; index++) {
      ...
      try {
        resolvedDeps.push(await this.resolveDependency(dep, moduleRef, requestContext));
      } catch (err: Error | any) {
    ...
    await this.resolveProperties(instance, metatype, moduleRef, requestContext);
  ...
  private async resolveFactory<T>(
    ...
    for (let index = 0; index < tokens.length; index++) {
      ...
      try {
        resolvedDeps.push(await this.resolveDependency(token, resolveContext, requestContext));
      } catch (err: Error | any) {
        if (optionalIndices.includes(index)) {
          resolvedDeps.push(undefined);
          continue;
  ...
  private async resolveDependency(
    ...
    // Request runtime values (for example Cloudflare Worker env/request/ctx)
    // are first-class DI values. They intentionally override any static
    // provider with the same token for the lifetime of this request.
    ...
    if (!wrapper.isResolved || wrapper.isTransient) {
      return this.resolveInstance(wrapper, host, requestContext);
    }
  ...
  /** Clear the internal resolution state associated with a request context. */
  public clearRequestContext(context: RequestContext): void {
    this.requestResolutionStacks.delete(context.id);
  }
  ...
  private async resolveProperties<T>(
    ...
    for (const prop of resolvedProperties) {
      ...
      try {
        const resolved = await this.resolveDependency(prop.type, moduleRef, requestContext);
   ...
   */
  private formatResolutionStack(token: InjectionToken, requestContext?: RequestContext): string {
    const stack = [...this.getResolutionStack(requestContext), token];
