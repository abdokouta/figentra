 ...
 * @description InstanceWrapper — Wraps a Provider Binding
 *
...
import { isRequestScope } from "@/core/contexts/request/request-scope";
...
export class InstanceWrapper<T = any> {
   ...
   * @default null
   */
  public inject: Array<InjectionToken | OptionalFactoryDependency> | null =
    null;
  ...
  /**
   * Request-scoped instances keyed by request context id.
   *
   * Kept on the wrapper so the injector can share request instances across
   * the dependency graph without mutating the application singleton.
   */
  private readonly requestInstances = new Map<string, T>();

  /** Get an instance for a request context, if one has been resolved. */
  public getRequestInstance(contextId: string): T | undefined {
    return this.requestInstances.get(contextId);
  }

  /** Cache an instance for a request context. */
  public setRequestInstance(contextId: string, instance: T): void {
    this.requestInstances.set(contextId, instance);
  }

  /** Dispose all request-scoped instances associated with this provider. */
  public clearRequestInstance(contextId: string): void {
  ...
  /**
   * Whether this provider is request-scoped.
   *
   * `REQUEST` was added to the contracts scope enum after the original
   * client-only container was designed. The defensive lookup keeps this
   * package compatible with older contracts while enabling the new scope
   * when the enum is present.
   */
  get isRequestScoped(): boolean {
    return isRequestScope(this.scope);
  }
