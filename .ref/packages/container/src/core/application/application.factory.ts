...
import { ApplicationBuilder } from "@/core/application/application-builder.service";
...
export class ApplicationFactory {
  ...
  public static async create(
    ...
    for (const moduleRef of container.getModules().values()) {
      if (!moduleRef.providers.has(ModuleContainer)) {
        container.addProvider(
          { provide: ModuleContainer, useValue: container },
          moduleRef.token,
        );
      }
    }
    ...
    for (const mod of container.getModules().values()) {
      if (!mod.providers.has(ModuleRef)) {
        ...
        container.addProvider(
          { provide: ModuleRef, useValue: moduleRefInstance },
          mod.token,
        );
      }
    }

    // ── Phase 4a: Instantiate ────────────────────────────────────────────
    // The framework-side boot-tracing logger (`ContainerLogger`) was
    // removed alongside the devtools eradication (commit `3c5c95a`);
    // consumers that want boot tracing wire their own observer via
    // `options.onReady` (fires after the DI graph is fully wired) or
    // through `@stackra/logger`. The DI graph itself never needs a
    // hard dependency on a logger — every phase logs to `console`
    // via `Debug` from `@stackra/support` when the runtime environment
    // opts in.
    if (!options.preview) {
      await context.init();
    }
    ...
    // Runtime adapters such as Cloudflare Workers can opt out because they
    // may host multiple application handlers in one isolate.
    if (options.registerGlobal ?? true) {
      setGlobalApplicationContext(context);
    }

    // ── Phase 6: Debug — expose on window ────────────────────────────────
    const { debug, globalName = DEFAULT_GLOBAL_NAME } = options;
    // Route env-var reads through `Env.isProduction()` from
    // `@stackra/support` per `support-utilities.md` §Rule 1. Handles
    // Node/Vite/window `__ENV__` triple-substrate lookup uniformly.
    const isDev = debug ?? !Env.isProduction();
  ...
  public static builder(
    rootModule: Type<any> | (() => Promise<Type<any>>),
  ): ApplicationBuilder {
