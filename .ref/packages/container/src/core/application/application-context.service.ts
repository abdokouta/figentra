 ...
 * @description ApplicationContext — The Bootstrapped DI Container
 *
 ...
 *
 *   The core is runtime-agnostic. HTTP/runtime adapters are intentionally kept
 *   outside the core, while request-scoped resolution is supported through
 *   child request contexts. The context provides:
 *
...
import { RequestContext, RequestContextFactory, RequestContextInput, RequestContextRegistry } from "@/core/contexts/request";
...
export class ApplicationContext {
  ...
  /** Factory and registry for per-request DI scopes. */
  private readonly requestContextFactory = new RequestContextFactory();
  private readonly requestContextRegistry = new RequestContextRegistry();
  ...
  public async init(): Promise<this> {
    ...
    // Flip the initialized flag BETWEEN phase 2 (onModuleInit) and
    // phase 3 (onApplicationBootstrap) via the loader's ready callback.
    // By phase 3, every provider is constructed AND initialised, so
    // `ModuleRef.get(...)` inside an `onApplicationBootstrap` hook MUST
    // work — that's the ADR-0052 §Canonical shape for `forFeature`
    // registrar classes. Flipping the flag after `createInstances`
    // returns is too late; phase 3 hooks fire inside it.
   ...
   */
  /**
   * Create a request-scoped child context.
   *
   * Runtime adapters use this to supply request-local values such as Request,
   * environment bindings, and execution contexts. REQUEST providers resolved
   * through the child are isolated from all other requests.
   */
  public createRequestContext(values: RequestContextInput = []): RequestApplicationContext {
    this.assertInitialized();
    const context = this.requestContextFactory.create(values);
    this.requestContextRegistry.register(context);
    return new RequestApplicationContext(this, context);
  }
  ...
  public async close(signal?: string): Promise<void> {
    const wrappers = [...this.container.getModules().values()].flatMap((m) => [...m.providers.values()]);
    for (const requestContext of [...this.requestContextRegistry.getAll()]) {
      this.requestContextRegistry.clear(requestContext, wrappers);
      this.instanceLoader.getInjector().clearRequestContext(requestContext);
    }
  ...
  /** @internal Resolve a token inside a request context. */
  public async resolveInRequestContext<T>(token: InjectionToken<T>, context: RequestContext): Promise<T> {
    if (context.has(token)) return context.get<T>(token) as T;

    for (const [, moduleRef] of this.container.getModules()) {
      const result = this.instanceLoader.getInjector().lookupProvider(token, moduleRef);
      if (!result) continue;
      return this.instanceLoader.getInjector().resolveInstance(result.wrapper, result.host, context);
    }
  ...
  /** @internal Close a request context and release request-scoped instances. */
  public async closeRequestContext(context: RequestContext): Promise<void> {
    const wrappers = [...this.container.getModules().values()].flatMap((m) => [...m.providers.values()]);
    this.requestContextRegistry.clear(context, wrappers);
    this.instanceLoader.getInjector().clearRequestContext(context);
  }
  ...
  private instantiateTransient<T>(wrapper: any, moduleRef: Module): T {
    ...
    const optionalIndices: number[] = (injector as any).getOptionalDependencies(
      metatype,
    );
 ...
 */
/**
 * Request-scoped facade over an ApplicationContext.
 */
export class RequestApplicationContext {
  public constructor(
    private readonly application: ApplicationContext,
    private readonly context: RequestContext,
  ) {}

  public get<T = any>(token: InjectionToken<T>): Promise<T> {
    return this.application.resolveInRequestContext(token, this.context);
  }

  public async getOptional<T = any>(token: InjectionToken<T>): Promise<T | undefined> {
    try {
  ...
  public has(token: InjectionToken): boolean {
    return this.context.has(token) || this.application.has(token);
  }
  ...
  public async close(): Promise<void> {
    await this.application.closeRequestContext(this.context);
  }
}
...
export class ScopedApplicationContext {
  ...
  public getContainer(): ModuleContainer {
    throw new Error(
      "getContainer() is not available on a scoped context. Use the root context.",
    );
  }
  ...
  public async close(): Promise<void> {
    throw new Error(
      "close() is not available on a scoped context. Use the root context.",
    );
  }
