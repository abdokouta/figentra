...
import { CoordinatorMessageKind } from "../../src/core/enums";
import { TabCoordinator } from "../../src/core/services/tab-coordinator.service";
import { MockTabTransportManager } from "../../src/testing/mock-tab-transport-manager";
import type { CoordinatorMessage } from "../../src/core/types";
...
describe("TabCoordinator", () => {
  ...
  describe("message handling", () => {
    ...
    it("leader responds to an ANNOUNCE with an immediate HEARTBEAT", () => {
      ...
      expect(
        seen.some((m) => m.kind === CoordinatorMessageKind.HEARTBEAT),
      ).toBe(true);
      coordinator.destroy();
    });
  });
