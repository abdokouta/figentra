...
import { CoordinatorTransport } from "../../src/core/services/coordinator-transport.service";
import { MockTabTransportManager } from "../../src/testing/mock-tab-transport-manager";
import type { IRelayMessage } from "../../src/core/interfaces/relay-message.interface";

const RELAY_CHANNEL = "stackra-event-relay";

/** Tiny sync event emitter shim — captures `emit` calls for assertion. */
function makeEmitter(): IEventEmitterSync & {
  calls: Array<[string | symbol, unknown[]]>;
} {
...
describe("CoordinatorTransport", () => {
  ...
  describe("connect + disconnect", () => {
    ...
    it("is inert when no manager was injected", () => {
      const transport = new CoordinatorTransport(undefined, {
        broadcastEvents: true,
      });
  ...
  describe("relay() pattern matching", () => {
    ...
    it("** matches one-or-more segments", () => {
      ...
      expect(seen.map((m) => m.event)).toEqual([
        "sync.pull",
        "sync.pull.batch",
      ]);
    });
  ...
  describe("inbound flow", () => {
    it("re-emits matching inbound relay messages on the local emitter", () => {
      ...
      expect(emitter.calls).toContainEqual([
        "auth.login",
        [{ userId: "u1" }, { at: 1 }],
      ]);
    });
