...
import type { CoordinatorMessage, RoleListener, TabRole } from "../types";
...
export class TabCoordinator {
  ...
  public constructor(
    ...
    private readonly eventEmitter?: IEventEmitterSync,
  ) {
    ...
    if (this.manager && this.manager.isSupported()) {
      this.transport = this.manager.channel(
  ...
  private claimLeadership(): void {
    if (
      ...
      return;
    this.postMessage({
      kind: CoordinatorMessageKind.CLAIM,
      tabId: this.tabId,
      at: Date.now(),
    });
    setTimeout(() => {
