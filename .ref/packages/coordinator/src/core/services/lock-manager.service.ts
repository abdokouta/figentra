...
export class LockManager {
  ...
  private async runWithWebLocks<T>(
    ...
    const lockOpts: { mode: "exclusive"; signal?: AbortSignal } = {
      mode: "exclusive",
    };
  ...
  private getStorageLock(key: string): { value: string; at: number } | null {
    // direct-localStorage-exempt: CAS primitive needs synchronous
    // read-then-write ordering (storage-usage.md §Exemption 1). SSR
    // + private-mode guard first, then a fail-soft catch that treats
    // any read failure as "no lock held".
  ...
  private setStorageLock(key: string, value: string): void {
    // direct-localStorage-exempt: CAS primitive needs synchronous
    // write immediately after the read above (storage-usage.md
    // §Exemption 1). Fail-soft on quota / disabled-storage errors —
    // the caller re-checks via getStorageLock() and retries.
  ...
  private clearStorageLock(key: string, expectedValue: string): void {
    // direct-localStorage-exempt: CAS release completes the
    // compare-and-swap started in getStorageLock/setStorageLock
    // (storage-usage.md §Exemption 1). Only removes when the stored
    // value still matches the acquiring owner's id — another tab may
    // have expired the lock and taken over.
    if (typeof localStorage === "undefined") return;
    try {
