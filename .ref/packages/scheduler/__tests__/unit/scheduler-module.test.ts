...
import { SchedulerModule } from "../../src/core/scheduler.module";
import { SchedulerService } from "../../src/core/services/scheduler.service";
import { DefaultTaskRunner } from "../../src/core/services/default-task-runner.service";
import { ScheduledTaskLoader } from "../../src/core/services/scheduled-task-loader.service";
import { MockTaskRunner } from "../../src/testing/mock-task-runner";
...
describe("SchedulerModule.forRoot", () => {
  it("binds config, service, and the discovery loader globally", () => {
    ...
    // Config provider.
    const cfgProvider = dyn.providers!.find(
      (p: any) => p.provide === SCHEDULER_CONFIG,
    ) as {
      useValue: unknown;
    };
    ...
    const alias = dyn.providers!.find(
      (p: any) => p.provide === SCHEDULER_SERVICE,
    ) as {
      useExisting: unknown;
    };
  ...
  it("defaults to DefaultTaskRunner via useClass", () => {
    const dyn = SchedulerModule.forRoot();
    const runnerProvider = dyn.providers!.find(
      (p: any) => p.provide === TASK_RUNNER,
    ) as {
      useClass?: unknown;
      useValue?: unknown;
    };
