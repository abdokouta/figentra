...
export class BrowserNetworkDetector implements INetworkDetector {
  ...
  public constructor() {
    // React Native ships a partial `window` polyfill so
    // `typeof window` returns "object" — but `addEventListener` is
    // not a function. Extend the SSR guard to also cover that case
    // (Hermes falls into it), otherwise the detector throws at
    // construction with "undefined is not a function" / "Invalid
    // event target". BrowserNetworkDetector is bound by
    // WebNetworkModule only; on native this class isn't
    // instantiated — but the guard defends against accidental
    // cross-import until Metro properly strips the react subpath.
    this.isSSR =
      typeof window === "undefined" ||
      typeof (window as { addEventListener?: unknown }).addEventListener !==
        "function";
...
/**
 * Structural view over the browser `Navigator` when the Network
 * Information API is present.
 *
 * `navigator.connection` is a Chromium-only, non-standard API — this
 * shim adds the property so the detector can guard-and-cast without
 * relying on a global type augmentation that would leak to consumers.
 */
export interface NavigatorWithConnection {
  /** The `NetworkInformation` snapshot, `undefined` on browsers that don't ship the API. */
