...
import { NETWORK_CONFIG } from "@stackra/contracts";
 ...
 */
export const networkConfig = registerAs<NetworkModuleOptions>(
  NETWORK_CONFIG,
  () => ({
    /*
  |--------------------------------------------------------------------------
  ...
  */
  ...
  |--------------------------------------------------------------------------
  |
  | Verbosity for network-related log messages. Overridden by the
  | centralized `logger.config.ts` in applications.
  |
  */
    logging: "info",
  ...
  |--------------------------------------------------------------------------
  |
  | Platform-specific network detector. Choose based on your
  | environment:
  |
  |   - BrowserNetworkDetector — Web (navigator.onLine + Network Info API)
  |   - NodeNetworkDetector    — Node.js (DNS polling)
  |   - NativeNetworkDetector  — React Native (@react-native-community/netinfo)
  |
  | Example:
  |
