...
export function createPermissionRoutes(): Hono<{
  ...
  router.get("/", async (c) => {
    ...
    const [actions, navigation, routes] = await Promise.all([
      c.env.DB.prepare(
        `SELECT a.slug AS application, v.version, aa.permission AS permission FROM application_actions aa JOIN applications a ON a.id = aa.application_id JOIN application_versions v ON v.id = aa.version_id ${where} AND aa.permission IS NOT NULL`,
      )
        .bind(...bindings)
        .all(),
      c.env.DB.prepare(
        `SELECT a.slug AS application, v.version, an.required_permission AS permission FROM application_navigation an JOIN applications a ON a.id = an.application_id JOIN application_versions v ON v.id = an.version_id ${where} AND an.required_permission IS NOT NULL`,
      )
        .bind(...bindings)
        .all(),
      c.env.DB.prepare(
        `SELECT a.slug AS application, v.version, ar.required_permission AS permission FROM application_routes ar JOIN applications a ON a.id = ar.application_id JOIN application_versions v ON v.id = ar.version_id ${where} AND ar.required_permission IS NOT NULL`,
      )
        .bind(...bindings)
        .all(),
    ]);
    ...
    const permissions = [...actions.results, ...navigation.results, ...routes.results]
      ...
      .filter(
        (row) =>
          ...
          !seen.has(`${row.application}:${row.version}:${row.permission}`),
      )
      .filter((row) => {
