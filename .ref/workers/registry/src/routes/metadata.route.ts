...
import { REGISTRY_CACHE_TTL_SECONDS } from "../constants/registry-cache-ttl.constant";
 ...
 */
export function createMetadataRoutes(): Hono<{
  ...
  router.get("/:slug/metadata", async (c) => {
    ...
    const application = await c.env.DB.prepare(
      ...
      .bind(slug)
      .first<{ id: string; current_version?: string }>();

    if (!application) return c.json({ error: "not_found" }, 404);

    const currentVersionId = await c.env.DB.prepare(
    ...
    const [capabilities, modules, resources, actions, navigation, environments, catalog] =
      await Promise.all([
        c.env.DB.prepare(
          ...
          .bind(application.id, currentVersionId.id)
          .all(),
        c.env.DB.prepare(
