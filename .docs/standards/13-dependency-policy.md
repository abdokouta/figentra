...
Named catalogs are used only when a package family must intentionally have a
...
A single service must not pin a version directly when the package exists in the
workspace catalog.
