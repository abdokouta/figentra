...
- Every PNG carries the same F-Cursor mark composed on a `#141414` (ink)
  background. Same brand tokens as `docs/brand/14-color-system.md`.
- Browser-tab favicons (16/32/64) render with sharp corners because the browser
  paints its own tab chrome around them.
- Touch/PWA icons (180/192/512) carry a `--radius-5`-equivalent rounded corner
  (24 units at the source 120 viewBox) so they read as consumer-app icons on
