...
export default function OnboardingWelcome(): React.ReactElement {
  return (
    <EmailShell preview="Welcome to Figentra. Here's the doctrine you're inheriting.">
      ...
      <P>
        The MSA is signed. Your <MonoChip>tenant-id</MonoChip> is provisioned. Your partner (
        {"{{partnerName}}"}) will run the intake call below. Before that, the doctrine you're
        inheriting:
      </P>

      <H2>What ships before code</H2>
      <P>
        Every commit at {"{{orgName}}"} passes through the same seven layers that ship every
        artefact on the Figentra platform: an ADR recording the architectural decision, a steering
        file codifying the per-repo rule, provenance frontmatter naming the author + reviewer, a
        human-signed commit, an automated CI gate, a review pass from a named lane owner, and a
        partner-countersigned deploy.
      </P>

      <Muted>
        None of this is optional. Every merge to your <MonoChip>main</MonoChip> branch runs the same
        seven layers.
      </Muted>
      ...
      <P>
        The intake call locks scope · timeline · governance envelope. Every subsequent artefact —
        the first ADR, the first migration, the first release note — traces back to what you scoped
        here.
      </P>
      ...
      <Muted>
        ...
        <span style={{ color: theme.ink }}>docs.figentra.cloud</span> — every steering file, every
        ADR, every runbook.
      </Muted>
    </EmailShell>
  );
