...
A service in this table is a **bounded responsibility**, not an automatic
deployment.

Start as a modular monolith or small number of deployables where appropriate.

Split into independently deployed services only when one or more of these are
true:
...
There is no standalone `public-api` Worker in the initial architecture. Public
API exposure is a routing/API-surface concern and can be served by the gateway,
registry, or the owning NestJS service.
