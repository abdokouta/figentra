...
**Status:** Baseline **Owner:** Platform architecture **Related:** every spec
(this is the one-page contract)
...
- Do not call the **Monetization** service "subscription-service" or "commerce"
  — [05 §1].
- Do not use **Next.js** as "the frontend framework" — it is Vite + RR7 — [13
  R-1].
...
3. **Supabase Auth Organization and Figentra Tenant are related but distinct**
   ([03]).
...
15. **Python is reserved for the AI service and model tooling** — not a general
    backend language.
...
29. **Applications ≠ Integrations; the Integration Platform is first-class**
    (R-9, [07]).
30. **Versioning is platform-wide via `@figentra/versioning`** — REST, webhooks,
    events, workflows, SDK, manifests (R-10, [12]).
...
> substrate, provisioned by Terraform, with Supabase Auth for identity and
> Stripe/Paddle behind a billing adapter.**
...
- A material change lands as an **ADR** that references the affected spec(s)
  and, where relevant, updates the
  [README reconciliation table](README.md#4-reconciliation-decisions-conv-vs-arch-doc)
  and the
  [open-questions register](README.md#7-consolidated-open-questions-register).
...
· O-4 billing-responsibility timing · O-5 Infra Orchestrator API scope · O-6
data-residency regions · O-7 first application.
