...
- `@figentra/scope-client` or the corresponding typed SDK contract for
  **scope**.
- `@figentra/policy-client` or the corresponding typed SDK contract for
  **policy**.
- `@figentra/audit-client` or the corresponding typed SDK contract for
  **audit**.
- `@figentra/approval-client` or the corresponding typed SDK contract for
  **approval**.
...
- **`role`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`permission`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`role_permission`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`principal_role`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`role_assignment`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`permission_grant`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`authorization_cache`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
...
- `iam.role.created.v1` — emitted only after the owning transaction commits
  through the outbox.
- `iam.role.updated.v1` — emitted only after the owning transaction commits
  through the outbox.
- `iam.assignment.created.v1` — emitted only after the owning transaction
  commits through the outbox.
- `iam.assignment.revoked.v1` — emitted only after the owning transaction
  commits through the outbox.
- `iam.permission.grant.created.v1` — emitted only after the owning transaction
  commits through the outbox.
- `iam.permission.grant.revoked.v1` — emitted only after the owning transaction
  commits through the outbox.
