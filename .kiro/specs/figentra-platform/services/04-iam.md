# IAM Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/iam.md`  
**Package:** `@figentra/iam`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Roles, permissions, grants and authorization decisions.
...
- `@figentra/scope-client` or the corresponding typed SDK contract for **scope**.
- `@figentra/policy-client` or the corresponding typed SDK contract for **policy**.
- `@figentra/audit-client` or the corresponding typed SDK contract for **audit**.
- `@figentra/approval-client` or the corresponding typed SDK contract for **approval**.
...
- **`role`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`permission`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`role_permission`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`principal_role`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`role_assignment`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`permission_grant`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`authorization_cache`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `role` | `role_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `permission` | `permission_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `role_permission` | `role_permission_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `principal_role` | `principal_role_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `role_assignment` | `role_assignment_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `permission_grant` | `permission_grant_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `authorization_cache` | `authorization_cache_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `iam.role.created.v1` — emitted only after the owning transaction commits through the outbox.
- `iam.role.updated.v1` — emitted only after the owning transaction commits through the outbox.
- `iam.assignment.created.v1` — emitted only after the owning transaction commits through the outbox.
- `iam.assignment.revoked.v1` — emitted only after the owning transaction commits through the outbox.
- `iam.permission.grant.created.v1` — emitted only after the owning transaction commits through the outbox.
- `iam.permission.grant.revoked.v1` — emitted only after the owning transaction commits through the outbox.
