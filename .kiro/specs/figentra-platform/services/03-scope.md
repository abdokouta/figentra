...
- **`scope`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`scope_type`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`scope_relation`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`scope_assignment`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`scope_closure`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
...
- `scope.created.v1` — emitted only after the owning transaction commits through
  the outbox.
- `scope.updated.v1` — emitted only after the owning transaction commits through
  the outbox.
- `scope.assignment.created.v1` — emitted only after the owning transaction
  commits through the outbox.
- `scope.assignment.revoked.v1` — emitted only after the owning transaction
  commits through the outbox.
