# Scope Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/scope.md`  
**Package:** `@figentra/scope`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Hierarchical business scope/context and assignment resolution.
...
- **`scope`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`scope_type`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`scope_relation`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`scope_assignment`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`scope_closure`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `scope` | `scope_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `scope_type` | `scope_type_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `scope_relation` | `scope_relation_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `scope_assignment` | `scope_assignment_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `scope_closure` | `scope_closure_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `POST /v1/tenants/:tenantId/scopes`
- `GET /v1/scopes/:id`
- `POST /v1/scopes/:id/children`
- `POST /v1/scopes/:id/assignments`
- `GET /v1/context`
...
- `scope.created.v1` — emitted only after the owning transaction commits through the outbox.
- `scope.updated.v1` — emitted only after the owning transaction commits through the outbox.
- `scope.assignment.created.v1` — emitted only after the owning transaction commits through the outbox.
- `scope.assignment.revoked.v1` — emitted only after the owning transaction commits through the outbox.
