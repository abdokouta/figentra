# Approval Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/approval.md`  
**Package:** `@figentra/approval`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Approval workflows, requests, decisions and escalation.
...
- `@figentra/workflow-client` or the corresponding typed SDK contract for **workflow**.
...
- **`approval_definition`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`approval_step`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`approval_request`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`approval_decision`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`approval_delegation`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`approval_escalation`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `approval_definition` | `approval_definition_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `approval_step` | `approval_step_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `approval_request` | `approval_request_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `approval_decision` | `approval_decision_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `approval_delegation` | `approval_delegation_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `approval_escalation` | `approval_escalation_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `approval.request.created.v1` — emitted only after the owning transaction commits through the outbox.
- `approval.request.approved.v1` — emitted only after the owning transaction commits through the outbox.
- `approval.request.rejected.v1` — emitted only after the owning transaction commits through the outbox.
- `approval.request.expired.v1` — emitted only after the owning transaction commits through the outbox.
