...
- `@figentra/policy-client` or the corresponding typed SDK contract for
  **policy**.
- `@figentra/workflow-client` or the corresponding typed SDK contract for
  **workflow**.
...
- **`approval_definition`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`approval_step`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`approval_request`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`approval_decision`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`approval_delegation`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`approval_escalation`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
...
- `approval.request.created.v1` — emitted only after the owning transaction
  commits through the outbox.
- `approval.request.approved.v1` — emitted only after the owning transaction
  commits through the outbox.
- `approval.request.rejected.v1` — emitted only after the owning transaction
  commits through the outbox.
- `approval.request.expired.v1` — emitted only after the owning transaction
  commits through the outbox.
