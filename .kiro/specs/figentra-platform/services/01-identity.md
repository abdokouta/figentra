# Identity Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/identity.md`  
...
**Purpose:** Identity and principal normalization over Supabase Auth.
...
- **`principal`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`identity_link`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`credential`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`service_account`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`integration_identity`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`identity_session`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`impersonation_grant`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `principal` | `principal_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `identity_link` | `identity_link_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `credential` | `credential_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `service_account` | `service_account_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `integration_identity` | `integration_identity_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `identity_session` | `identity_session_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `impersonation_grant` | `impersonation_grant_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `POST /v1/identity/token-exchange`
- `POST /v1/identity/impersonations`
...
- `identity.principal.created.v1` — emitted only after the owning transaction commits through the outbox.
- `identity.principal.updated.v1` — emitted only after the owning transaction commits through the outbox.
- `identity.service_account.created.v1` — emitted only after the owning transaction commits through the outbox.
- `identity.credential.rotated.v1` — emitted only after the owning transaction commits through the outbox.
- `identity.impersonation.started.v1` — emitted only after the owning transaction commits through the outbox.
- `identity.impersonation.ended.v1` — emitted only after the owning transaction commits through the outbox.
