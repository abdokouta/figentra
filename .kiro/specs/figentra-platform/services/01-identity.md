...
- **`principal`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`identity_link`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`credential`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`service_account`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`integration_identity`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`identity_session`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`impersonation_grant`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
...
- `identity.principal.created.v1` — emitted only after the owning transaction
  commits through the outbox.
- `identity.principal.updated.v1` — emitted only after the owning transaction
  commits through the outbox.
- `identity.service_account.created.v1` — emitted only after the owning
  transaction commits through the outbox.
- `identity.credential.rotated.v1` — emitted only after the owning transaction
  commits through the outbox.
- `identity.impersonation.started.v1` — emitted only after the owning
  transaction commits through the outbox.
- `identity.impersonation.ended.v1` — emitted only after the owning transaction
  commits through the outbox.
