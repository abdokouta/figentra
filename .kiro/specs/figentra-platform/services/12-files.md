...
**Purpose:** Object metadata, upload authorization, downloads, lifecycle and R2
integration.
...
- **`file_object`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`file_version`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`file_part`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`upload_session`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`file_access_grant`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`file_link`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
...
- `file.created.v1` — emitted only after the owning transaction commits through
  the outbox.
- `file.completed.v1` — emitted only after the owning transaction commits
  through the outbox.
- `file.deleted.v1` — emitted only after the owning transaction commits through
  the outbox.
- `file.expired.v1` — emitted only after the owning transaction commits through
  the outbox.
