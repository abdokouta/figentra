...
- **`notification`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`notification_template`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`notification_recipient`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`delivery_attempt`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`provider`** — aggregate/domain record. Define lifecycle, invariants, state
...
- **`webhook_delivery`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`preference`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
...
- `notification.created.v1` — emitted only after the owning transaction commits
  through the outbox.
- `notification.sent.v1` — emitted only after the owning transaction commits
  through the outbox.
- `notification.failed.v1` — emitted only after the owning transaction commits
  through the outbox.
- `notification.delivery.retry.v1` — emitted only after the owning transaction
