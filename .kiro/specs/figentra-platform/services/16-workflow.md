...
**Purpose:** Durable workflow definitions, runs, steps, retries and compensation
facade.
...
- **`workflow_definition`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`workflow_version`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`workflow_run`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`workflow_step`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`workflow_event`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`workflow_compensation`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
...
- `workflow.run.started.v1` — emitted only after the owning transaction commits
  through the outbox.
- `workflow.step.completed.v1` — emitted only after the owning transaction
  commits through the outbox.
- `workflow.run.completed.v1` — emitted only after the owning transaction
  commits through the outbox.
- `workflow.run.failed.v1` — emitted only after the owning transaction commits
  through the outbox.
- `workflow.run.cancelled.v1` — emitted only after the owning transaction
  commits through the outbox.
