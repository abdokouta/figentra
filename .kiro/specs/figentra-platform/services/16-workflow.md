# Workflow Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/workflow.md`  
**Package:** `@figentra/workflow`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Durable workflow definitions, runs, steps, retries and compensation facade.
...
- **`workflow_definition`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`workflow_version`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`workflow_run`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`workflow_step`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`workflow_event`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`workflow_compensation`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `workflow_definition` | `workflow_definition_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `workflow_version` | `workflow_version_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `workflow_run` | `workflow_run_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `workflow_step` | `workflow_step_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `workflow_event` | `workflow_event_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `workflow_compensation` | `workflow_compensation_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `workflow.run.started.v1` — emitted only after the owning transaction commits through the outbox.
- `workflow.step.completed.v1` — emitted only after the owning transaction commits through the outbox.
- `workflow.run.completed.v1` — emitted only after the owning transaction commits through the outbox.
- `workflow.run.failed.v1` — emitted only after the owning transaction commits through the outbox.
- `workflow.run.cancelled.v1` — emitted only after the owning transaction commits through the outbox.
