...
- **`search_definition`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`search_index`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`search_document`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`index_job`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`index_failure`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
...
- `search.index.updated.v1` — emitted only after the owning transaction commits
  through the outbox.
- `search.reindex.requested.v1` — emitted only after the owning transaction
  commits through the outbox.
- `search.index.failed.v1` — emitted only after the owning transaction commits
  through the outbox.
