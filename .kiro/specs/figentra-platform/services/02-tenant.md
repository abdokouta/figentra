...
**Purpose:** Tenant lifecycle, organization mapping, domains, branding and
configuration.
...
- `@figentra/identity-client` or the corresponding typed SDK contract for
  **identity**.
- `@figentra/scope-client` or the corresponding typed SDK contract for
  **scope**.
- `@figentra/iam-client` or the corresponding typed SDK contract for **iam**.
- `@figentra/monetization-client` or the corresponding typed SDK contract for
  **monetization**.
- `@figentra/notifications-client` or the corresponding typed SDK contract for
  **notifications**.
- `@figentra/registry-client` or the corresponding typed SDK contract for
  **registry**.
...
- **`tenant`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`tenant_member`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`tenant_application`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`domain`** — aggregate/domain record. Define lifecycle, invariants, state
  transitions, actor attribution, tenant ownership and soft-delete policy before
  implementation.
- **`domain_verification`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`tenant_setting`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`tenant_branding`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
...
- `tenant.created.v1` — emitted only after the owning transaction commits
  through the outbox.
- `tenant.updated.v1` — emitted only after the owning transaction commits
  through the outbox.
- `tenant.suspended.v1` — emitted only after the owning transaction commits
  through the outbox.
- `tenant.deleted.v1` — emitted only after the owning transaction commits
  through the outbox.
- `tenant.domain.added.v1` — emitted only after the owning transaction commits
  through the outbox.
- `tenant.domain.verified.v1` — emitted only after the owning transaction
  commits through the outbox.
- `tenant.domain.activated.v1` — emitted only after the owning transaction
  commits through the outbox.
