# Tenant Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/tenant.md`  
**Package:** `@figentra/tenant`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Tenant lifecycle, organization mapping, domains, branding and configuration.
...
- `@figentra/registry-client` or the corresponding typed SDK contract for **registry**.
...
- **`tenant`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`tenant_member`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`tenant_application`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`domain`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`domain_verification`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`tenant_setting`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`tenant_branding`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `tenant` | `tenant_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `tenant_member` | `tenant_member_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `tenant_application` | `tenant_application_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `domain` | `domain_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `domain_verification` | `domain_verification_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `tenant_setting` | `tenant_setting_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `tenant_branding` | `tenant_branding_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `tenant.created.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.updated.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.suspended.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.deleted.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.domain.added.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.domain.verified.v1` — emitted only after the owning transaction commits through the outbox.
- `tenant.domain.activated.v1` — emitted only after the owning transaction commits through the outbox.
