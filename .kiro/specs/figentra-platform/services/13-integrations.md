# Integrations Service — Kiro Implementation Specification

**Spec:** `figentra-platform/services/integrations.md`  
**Package:** `@figentra/integrations`  
**Status:** Baseline target / implementation specification  
**Runtime:** Node.js 22 + NestJS in Cloudflare Containers  

## 1. Mission and boundary

**Purpose:** Integration catalog, installations, credentials, mappings and webhooks.
...
- `@figentra/files-client` or the corresponding typed SDK contract for **files**.
...
- **`integration`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`integration_version`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`integration_installation`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`integration_config`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`credential_reference`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`connection`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`webhook_endpoint`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`webhook_delivery`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
- **`mapping`** — aggregate/domain record. Define lifecycle, invariants, state transitions, actor attribution, tenant ownership and soft-delete policy before implementation.
...
|---|---|---|---|---|
| `integration` | `integration_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `integration_version` | `integration_version_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `integration_installation` | `integration_installation_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `integration_config` | `integration_config_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `credential_reference` | `credential_reference_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `connection` | `connection_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `webhook_endpoint` | `webhook_endpoint_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `webhook_delivery` | `webhook_delivery_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
| `mapping` | `mapping_id` (ULID/domain prefix) | tenant_id where tenant-owned | created_at, updated_at | owning aggregate fields; unique/indexes as required |
...
- `GET /v1/integrations`
- `POST /v1/integrations/:key/install`
- `POST /v1/integrations/:key/connect`
- `POST /v1/integrations/:id/pause`
- `POST /v1/integrations/webhooks/:key`
...
- `integration.installed.v1` — emitted only after the owning transaction commits through the outbox.
- `integration.connected.v1` — emitted only after the owning transaction commits through the outbox.
- `integration.paused.v1` — emitted only after the owning transaction commits through the outbox.
- `integration.webhook.received.v1` — emitted only after the owning transaction commits through the outbox.
- `integration.webhook.failed.v1` — emitted only after the owning transaction commits through the outbox.
