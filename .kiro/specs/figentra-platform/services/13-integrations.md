...
**Purpose:** Integration catalog, installations, credentials, mappings and
webhooks.
...
- `@figentra/tenant-client` or the corresponding typed SDK contract for
  **tenant**.
- `@figentra/files-client` or the corresponding typed SDK contract for
  **files**.
...
- **`integration`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`integration_version`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`integration_installation`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`integration_config`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`credential_reference`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`connection`** — aggregate/domain record. Define lifecycle, invariants,
  state transitions, actor attribution, tenant ownership and soft-delete policy
  before implementation.
- **`webhook_endpoint`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`webhook_delivery`** — aggregate/domain record. Define lifecycle,
  invariants, state transitions, actor attribution, tenant ownership and
  soft-delete policy before implementation.
- **`mapping`** — aggregate/domain record. Define lifecycle, invariants, state
...
- `integration.installed.v1` — emitted only after the owning transaction commits
  through the outbox.
- `integration.connected.v1` — emitted only after the owning transaction commits
  through the outbox.
- `integration.paused.v1` — emitted only after the owning transaction commits
  through the outbox.
- `integration.webhook.received.v1` — emitted only after the owning transaction
  commits through the outbox.
- `integration.webhook.failed.v1` — emitted only after the owning transaction
  commits through the outbox.
