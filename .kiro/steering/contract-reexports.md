...
## Rule — never re-export a peer's symbols

A workspace package MUST NOT re-export symbols it consumed from a peer
dependency's public API (`@heroui/react`, `@heroui-pro/react`, `react`,
`react-dom`, `@tanstack/react-query`, `@tanstack/store`, `@stackra/routing`,
`@stackra/ui`, ...). Consumers of your package import from the peer directly.

```ts
// ❌ WRONG — @stackra/rbac forwarding a HeroUI export.
export { Button } from "@heroui/react";
```

```ts
// ✅ RIGHT — consumers import HeroUI (via @stackra/ui) themselves.
// Nothing to re-export from @stackra/rbac.
```

### Why

- **Consumers who skip your package can still import from the peer.**
  Re-exporting doubles the public surface without adding value.
- **Peer swaps break the shim.** If we replace HeroUI or TanStack Query with
  something else, every package that re-exports them becomes a compat shim
  maintained forever.
- **Version drift.** Two different re-exports of `Button` from two different
  `@stackra/*` packages read the same type — until one bumps the peer minor
  version and the types diverge.

### Exception — the package IS the surface

`@stackra/ui` re-exports HeroUI intentionally — the package's whole purpose IS
"the workspace's HeroUI surface". Same for `@stackra/state` re-exporting
`@tanstack/store` (the package's whole purpose IS the TanStack Store
integration) and `@stackra/http/rxjs` re-exporting RxJS.

The rule of thumb: if a consumer imports your package solely to reach the peer,
the re-export is the point. Otherwise, don't re-export.

Enforced by review + the audit greps below.
