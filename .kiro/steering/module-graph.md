...
- `versioning` (12) — public-interface versioning (REST + payload + webhook +
  GraphQL). Distinct from foundation's row-level versioning helper.
- `telemetry` (12) — three-signal observability (traces + metrics + logs). See
  the module readme for the OpenTelemetry + structured-logging stack.
...
- `subscription` (29) — plan definitions + billing-provider (Stripe)
  integration. Depends on `entitlements` (default_entitlements sync on plan
  swap) + `notifications-mail` (welcome + upgrade notifications), which is why
  its priority is highest in this wave.
...
- `localization` — i18n wrapper + machine-translation drivers. Publishes the
  per-row translation helper + a `translatable` field annotation. See
