...
frontend work and mirrors the backend module architecture so the two codebases
stay conceptually 1:1.

> **Scope.** These rules apply to APP-TIER code under `apps/*/src/` — the
> customer-facing SPAs the workspace ships (academorix-dashboard,
> academorix-landing). They do NOT apply to workspace framework packages under
> `frontend/packages/*/src/`; framework packages follow
> `.kiro/steering/frontend-packages.md` which BANS `@refinedev/*`. The
> framework/app boundary is codified by
> [ADR-0060](../../docs/adr/0060-refine-dev-framework-app-boundary.md). Refine
> adoption here (`useResource`, `useTable`, `ResourceDataGrid`, action buttons)
> composes on top of `@stackra/query` as the underlying `dataProvider` — never
> bypasses the framework's DI-first primitives.
...
│   ├── query/ http/ api/    # query builder, HTTP client, OpenAPI types
...
Resource icons are **components**, typed as `IconType` from `@stackra/ui/icons`
(heroicons-compatible). Manifests pass the component (`icon: AcademicCapIcon`);
the layout renders it with consistent sizing. Never pass a pre-rendered element.
...
backend's tenant-identification model:

- **Central** (`figentra.com`) — workspace picker, marketing, self-serve tenant
  creation.
- **Central admin** (`admin.figentra.com`) — Stackra staff surface;
  `platform.domain` middleware rejects tenant hosts.
- **Tenant** (`{slug}.figentra.com`, custom domains, dev localhost) — every
...
provider built on top of `httpClient` (bearer-token-authenticated, host-scoped
by `resolveHostContext()`). Every Refine resource resolves through it; RPC- style
...
Providers live in `src/providers/` and target the backend REST API exclusively.
...
Canonical response contract (the backend wraps resource responses in a `data`
envelope):
...
- Query contract follows a JSON:API-style query-string convention:
  `page` + `per_page` (offset paginator), `sort=-created_at,name`,
  ...
  decides exact vs partial per field), and `filter[field][<op>]=value` for
  comparison/range/presence operators (`gte`, `lte`, `between`, `null`, …) which
  a single operator filter interprets. See `lib/query/query.ts`.
