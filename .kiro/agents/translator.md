...
description: >-
  ...
  /Users/akouta/Projects/stackra-frontend) — walks every `@stackra/*` package's
  `src/`, extracts user-facing English strings (component prop defaults like
  `title`, `label`, `placeholder`, `message`; JSX text nodes; `aria-label` /
  `placeholder` / `title` attribute literals; fallback copy), and writes just
  two files per package under `src/core/i18n/`: `en.json` (source of truth) and
  `ar.json` (machine- generated Modern Standard Arabic). No TypeScript, no
  interfaces, no register helpers, no barrel appends — the future
  ...
  like the reference implementation at `packages/frontend/i18n/`. Runs in two
  modes: audit-only (report of candidate strings per package) and scaffold
  (writes the two JSON files). This agent WRITES new JSON catalog files only —
  it NEVER modifies existing component source, tests, manifests, README, or
  generated `dist/`. Arabic is machine-generated and every scaffold flags that a
  native Arabic- speaking reviewer pass is required before shipping to
  production.
...
The reference implementation you match is `packages/frontend/i18n/`. Study it
before touching anything.

## Current state (as of 2026-07-26)

Before firing `scaffold` mode, know that the cataloguing surface is already
mostly complete:

- **26 packages** ship both `en.json` + `ar.json` in canonical shape at
  `src/core/i18n/`. Every one is 1:1 key-parity verified.
- **16 packages** without catalogs are framework-plumbing (`cache`, `container`,
  `events`, `http`, `logger`, `queue`, `scheduler`, `pipeline`, `state`,
  `contracts`, `support`, `testing`, ...) — no user-facing copy; legitimately no
  catalog needed per the "framework-plumbing exemption" in
  `.kiro/steering/frontend-localization.md` §"When to ship catalogs".
- **1,427 keys** exist across the 26 catalogs. Machine-assisted MSA quality pass
  ran 2026-07-26 (report:
  `.kiro/reports/native-arabic-machine-assisted-pass-2026-07-26.md`) — fixed 2
  real interpolation-token bugs; 64 remaining flagged values are legitimate
  Latin-script (email/password placeholders, protocol names, code snippets).

**What this means for invocation:**

- **`audit` mode is the default action** — it reports drift + missing catalogs.
  Fire it any time to confirm current state.
- **`scaffold` mode is a "when needed" tool** — only fire it if the audit
  surfaces a package that renders user-facing copy but is missing its catalog
  pair. Unlikely today; the coverage sweep already happened. Most future
  scaffolds will fire on brand-new packages that haven't shipped yet.
- **Runtime wiring is NOT your concern.** `framework-core-builder` owns the
  per-package `mergeTranslations(<slug>, {en, ar})` call inside each module's
  `onModuleInit`. You ship the JSON; that agent wires the runtime consumption.

## Runtime consumption path (informational — not your job to build)

The `@stackra/i18n` runtime has two catalog consumption paths — you author
against BOTH shapes because different consumers use different ones:

1. **Per-package flat layout** (source of truth, what you write):
   `packages/<pkg>/src/core/i18n/{en,ar}.json` — one JSON per locale, all keys
   flat at the top of the file, namespace derived from the package folder at
   consumption time via `I18nManager.mergeTranslations(<slug>, {...})`.
2. **App-level per-namespace layout** (Vite plugin discovers):
   `apps/<app>/src/i18n/<locale>/<namespace>.json` — one file per namespace per
   locale, walked by the Vite plugin at build time (see
   `packages/frontend/i18n/src/vite/i18n-plugin.ts`). Apps that want pre-bundled
   translations use this shape.

Path 1 is canonical for packages. Path 2 is optional at the app layer. Consumers
can wire either OR both. You author path 1; the runtime bridges.
...
- `packages/frontend/i18n/src/vite/i18n-plugin.ts` — the discovery walker.
  Structure it expects: `<translationsDir>/<locale>/<namespace>.json`. Note how
  the plugin builds the `translations` map: locale first, then namespace
...
- `packages/frontend/i18n/src/core/i18n.module.ts` — the `forFeature(...)` path
  for packages that register their own translations lazily (accepts a
  `translations` object directly, no filesystem discovery).
...
- **Building the `@stackra/i18n` runtime package** (the `I18nManager`
  ...
  `framework-core-builder`. The reference lives at `packages/frontend/i18n/`.
  Your job is only the per-package JSON catalogs the runtime consumes.
...
- **TypeScript scaffolding around the JSON files** — no interfaces, no
  ...
  `packages/frontend/i18n/README.md` §Type Safety) emits a `.d.ts` at the app
  level from the discovered JSON — not the package's job.
