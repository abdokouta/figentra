...
`@stackra/testing` is a BUILD-TOOL PRESET (its `./preset` entry is merged into
every workspace package's `vitest.config.ts`). Per
[ADR-0051 — CLI-package `dependencies` block exception](../../../docs/adr/0051-cli-package-dependencies-exception.md),
the two deps below are declared under `dependencies` (not `peerDependencies`)
because they are build-time only — the preset itself is the sole caller, and
consuming packages never `import` from them.

| Dependency     | Consumed by                                                                             |
| -------------- | --------------------------------------------------------------------------------------- |
| `@swc/core`    | SWC compiler used inside the preset for legacy decorator + decorator metadata emission. |
| `unplugin-swc` | Vite plugin wiring SWC into Vitest 4 (replaces the default OXC / esbuild transformers). |

Both deps are pinned via the workspace `catalog:` protocol so version drift is
impossible.
