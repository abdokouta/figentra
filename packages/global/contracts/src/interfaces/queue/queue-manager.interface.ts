 ...
 *   `@figentra/queue` implements this + additional per-connection
 ...
 *   without pulling `@figentra/queue` in as a hard dependency.
 *
 ...
 *   they inject the concrete `QueueManager` class from `@figentra/queue`.
 */

/**
 * Optional shape for job-dispatch options at the framework contract
 * level. Concrete implementations (see `IJobOptions` in
 * `@figentra/queue`) accept a broader set — this shape is the
 ...
 * Concrete `@figentra/queue` `QueueManager` satisfies this shape
