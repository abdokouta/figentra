...
> {
   ...
   * package's runtime resolver appends the `"auth"` guard to the
   * effective `guards` array (inner position). The concrete gate
   * class is resolved from `GuardRegistry` at request time —
   * consumers register it via `@Guard({ name: "auth" })` or via
   * `RoutingModule.registerGuard(...)`.
   *
   * Codified by ADR-0096 — Declarative route guards via
   * `@stackra/routing` registry.
   *
   * @default undefined (no auth guard)
   */
  readonly authed?: boolean;

  /**
   * Declarative tenant-gate shortcut. When `true`, the runtime
   * resolver PREPENDS the `"tenant"` guard to the effective
   * `guards` array (outer position). Composes with `authed` per
   * `auth-tenancy-composition.md` §Rule 3: tenanted (outer) →
   * authed (inner). Never the reverse.
   *
   * @default undefined (no tenant guard)
   */
  readonly tenanted?: boolean;

  /**
   * Declarative public-route marker. When `true`, the runtime
   * resolver CLEARS every guard (route-level + parent-inherited)
   * for this route. Escape hatch for public-by-design routes
   * nested under a guarded parent.
   *
   * Overrides `authed` + `tenanted` even if either is `true`.
   *
   * @default undefined (parent + route guards apply)
   */
   ...
   */
  readonly announce?:
    string | ((ctx: IPageContext<TData, TParams>) => string) | false;
}
