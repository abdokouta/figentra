...
export interface ILayoutConfig {
  ...
  /**
   * Declarative auth-gate shortcut — see IPageConfig.authed. Layouts
   * MAY declare shorthand guards; every child route inherits.
   */
  readonly authed?: boolean;

  /**
   * Declarative tenant-gate shortcut — see IPageConfig.tenanted.
   */
  readonly tenanted?: boolean;

  /**
   * Declarative public-route marker — see IPageConfig.public.
   * When set on a LAYOUT, every child route inherits the "skip
   * guards" behaviour; children can override with their own
   * `public: false` (rare — usually you `public: true` a specific
   * child under a guarded parent).
   */
