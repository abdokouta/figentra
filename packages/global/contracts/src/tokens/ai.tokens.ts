...
/**
 * Configuration namespace for the AI subsystem.
 *
 * String constant used both as the `registerAs(AI_CONFIG, ...)`
 * namespace on the app-side config factory AND as the DI token that
 * `AiModule` binds the resolved config under. The value IS the
 * namespace string — consumers can spell either the constant or the
 * literal `"ai"` and reach the same registration.
 */
export const AI_CONFIG = "ai" as const;
