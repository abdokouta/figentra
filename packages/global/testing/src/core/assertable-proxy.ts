 ...
 *
 * @deprecated Prefer `AssertableProxy` from `@stackra/contracts` for
 *   cross-package typing. This local alias is kept as a runtime shim for
 *   backward compatibility — it narrows the `$` accessor to the concrete
 *   `Assertable` class (whereas the contracts version narrows to
 *   `IAssertable`). New code that needs cross-package portability should
 *   import from `@stackra/contracts` directly.
 */
  ...
  const handler: ProxyHandler<T> = {
    get(obj, prop, receiver) {
      ...
      return (...args: unknown[]): unknown => {
        ...
        try {
          const returnValue = (value as (...args: unknown[]) => unknown).apply(
            obj,
            args,
          );
          assertable.record({ method, args, returnValue });
