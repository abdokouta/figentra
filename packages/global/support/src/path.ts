...
// where Vite externalises `node:path` to a throwing stub AND in
// React Native bundles where Metro has no `node:path` shim at all.
//
...
//
// `node:url` is INTENTIONALLY NOT imported here — see the
// `urlToPath` docblock below. The WHATWG `URL` global (a Web
// standard available on every runtime `Path` touches — Node,
// browser, React Native / Hermes) handles every case the previous
// `urlModule.fileURLToPath()` fast-path did, including percent-
// decoded characters (via `decodeURIComponent`) and Windows drive-
// letter fixups. Dropping the Node import kills the Metro-bundler
// hazard (Metro has no `node:url` shim) and simplifies the code path
// to a single implementation on every runtime.
 ...
 * that works on every runtime `Path` touches — Node, browser
 * (Vite/webpack), and React Native (Metro).
 *
 * Single-stage resolution via the WHATWG `URL` class:
 *
 *   1. **Parse via `new URL(...)`** — a Web standard, available on
 *      every runtime. `.pathname` gives us the path portion for any
 *      URL scheme (`file://`, `http://`, `https://`, ...), so a
 *      browser module whose `import.meta.url` is an `http://`
 *      served-source URL still yields a workable path string.
 *
 *   2. **Percent-decode** — `URL.pathname` returns the ENCODED path
 *      form (`file:///foo%20bar` → `pathname === "/foo%20bar"`).
 *      Node's `url.fileURLToPath()` decodes these; matching that
 *      behaviour keeps directories with spaces / unicode working.
 *
 *   3. **Windows drive-letter fixup** — `file:///C:/foo` yields
 *      `pathname === "/C:/foo"`; strip the leading slash so
 *      downstream `path.dirname` etc. see a well-formed drive path.
 *      POSIX paths pass through unchanged.
 *
 *   4. **Last resort** — return an empty string. Never throws.
 *      Callers get a benign no-op value in exotic runtimes.
 *
 * ## Why no `node:url` import
 *
 * `url.fileURLToPath()` is the Node-native way to do this and used
 * to be the fast-path here (wrapped in try/catch). Two forces killed
 * it:
 *
 * - Vite/webpack v7+ externalise `node:url` in browser bundles —
 *   the stub throws on every property access, so the try/catch
 *   fires on every call site.
 * - Metro (React Native's bundler) has no `node:url` shim at all —
 *   static `import * as urlModule from "node:url"` becomes an
 *   unresolvable specifier and blows up bundling.
 *
 * The WHATWG URL fallback already handles every scenario Node's
 * fast-path did, so keeping the Node import earned nothing and cost
 * both bundler ecosystems a workaround. Removing it deletes the
 * workaround.
 *
...
function urlToPath(importMetaUrl: string): string {
  try {
    const raw = new URL(importMetaUrl).pathname;
    ...
    // through unchanged.
    const stripped = /^\/[A-Za-z]:\//.test(raw) ? raw.slice(1) : raw;

    // Percent-decode — matches Node's `fileURLToPath` behaviour
    // so paths with spaces or unicode work uniformly.
...
export class Path {
  ...
  public static setRoot(root: string): void {
    if (typeof root !== "string" || root.length === 0) {
      ...
      throw new TypeError(
        `Path.setRoot: expected a non-empty string, got ${typeof root}`,
      );
    }
