  ...
  describe("shape", () => {
    it("returns a function", async () => {
      const { createDefineConfig } =
        await import("../../src/utils/create-define-config.util");
  ...
  describe("type — bound alias narrows to TOptions", () => {
    it("typed alias enforces the bound shape at compile time", async () => {
      const { createDefineConfig } =
        await import("../../src/utils/create-define-config.util");
      const defineCacheConfig = createDefineConfig<CacheOptions>();
      expectTypeOf(defineCacheConfig).toEqualTypeOf<
    ...
    it("two aliases have independent parameter types", async () => {
      ...
      expectTypeOf(defineCacheConfig)
        .parameter(0)
        .toEqualTypeOf<CacheOptions>();
      expectTypeOf(defineQueueConfig)
        .parameter(0)
        .toEqualTypeOf<QueueOptions>();
    });
  });
});
