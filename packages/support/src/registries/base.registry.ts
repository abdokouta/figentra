 ...
 * ```
 *
 * @example Hook-style attach / detach
 * ```typescript
 * // In a React hook — attach returns an idempotent unregister
 * // closure so useEffect cleanup is one-line.
 * useEffect(() => registry.attach(command.id, command), [command]);
 * ```
 */
export abstract class BaseRegistry<TKey, TValue> {
  ...
  /**
   * Register a value and return an unregister closure.
   *
   * Convenience for hook-style call sites (React `useEffect`
   * cleanup, subscription hooks) that want to unregister when
   * the component unmounts:
   *
   *   const unregister = registry.attach(command.id, command);
   *   useEffect(() => unregister, []);
   *
   * Uses `replace` internally so re-attaching the same key
   * never throws — matches the last-write-wins semantics
   * needed for hot-reload scenarios where a hook body re-runs
   * with the same identity.
   *
   * The returned closure is idempotent — calling it more than
   * once is a no-op after the first call (`remove` returns
   * `false` when the key is gone).
   *
   * @param key - The registry key.
   * @param value - The value to register.
   * @returns Idempotent unregister function that calls
   *   {@link remove}. Safe to invoke multiple times.
   *
   * @example
   * ```typescript
   * // Inside a React hook — unregister on unmount.
   * useEffect(() => registry.attach(command.id, command), [command]);
   * ```
   */
  public attach(key: TKey, value: TValue): () => void {
  ...
  /**
   * Attach a batch of entries atomically.
   *
   * Semantics match calling {@link attach} for each entry; the
   * returned unregister function removes only the entries
   * registered by THIS call. Entries later re-registered by a
   * separate call survive.
   *
   * The unregister closure closes over the batch's keys — so if
   * a later `attach(sameKey, otherValue)` replaces one of the
   * entries, this closure will still `remove` the current value
   * at that key. Registry keys are expected to be stable + unique
   * by contract, so a re-register at the same key is treated as a
   * continuation of the same slot, not a new one.
   *
   * @param entries - Array of `[key, value]` tuples.
   * @returns Idempotent unregister function.
   *
   * @example
   * ```typescript
   * useEffect(
   *   () => registry.attachAll(entries.map((e) => [e.id, e])),
   *   [entries],
   * );
   * ```
   */
  public attachAll(entries: readonly (readonly [TKey, TValue])[]): () => void {
    const keys = entries.map(([key]) => key);
    for (const [key, value] of entries) this.replace(key, value);
    return () => {
