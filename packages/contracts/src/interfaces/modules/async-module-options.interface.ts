 ...
 * @description Canonical async-options shape for NestJS dynamic modules
 *   that register a single resolved configuration via `forRootAsync()` /
 *   `forFeatureAsync()`.
 *
 ...
 *   unsafe variadic-tuple gymnastics. Adopting `any[]` here matches
 *   NestJS first-party modules and is the canonical pattern.
 *
...
/**
 * Canonical async-options shape accepted by every workspace module's
 * `forRootAsync(...)` and `forFeatureAsync(...)` static method — mirrors
 * `IAsyncModuleOptions` from `@nestjs/common` with the `useFactory` +
 * `inject` + `imports` triple.
 *
 * @typeParam T - Shape of the resolved options the factory produces.
 */
