...
import type { ILocaleItem } from "./locale-item.interface";
...
export interface II18nLocaleService {
  ...
  /**
   * Display metadata for every supported locale — one item per
   * BCP-47 code with an optional human name + flag. Consumers
   * render `[flag] [name]` from this array; when the app didn't
   * supply an entry the shape falls back to `{ code, name: code }`
   * so the picker never renders a blank label.
   */
  getLocales(): readonly ILocaleItem[];
