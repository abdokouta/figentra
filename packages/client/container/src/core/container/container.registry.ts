 ...
 * @description ModuleContainer — Top-Level Module Registry
 *
 ...
 *
 * @support-utilities-exempt The DI container itself. {@link BaseRegistry}
 *   lives in `@stackra/support` which depends on `@stackra/container` —
 *   extending {@link BaseRegistry} would create a circular dependency.
 *   Additionally, this class holds modules keyed by opaque token with
 *   providers, imports, exports, and dynamic metadata — a different
 *   shape from key→value.
 *
 * @example
 * ```typescript
...
export class ModuleContainer {
  ...
  public async addModule(
    ...
    const { type, dynamicMetadata, token } =
      this.extractModuleMetadata(resolved);
    ...
    if (this.modules.has(token)) {
      ...
      // Upgrade to global scope if a later contribution declares
      // `global: true`. This handles the DFS-order case where a
      // module was first registered by a `forFeature(...)` (no
      // global flag) and later composed via `forRoot()` (global).
      // Without this, `bindGlobalScope()` never links the module
      // to consumers, and DI resolution fails for any consumer that
      // doesn't explicitly `imports: [X.forRoot()]` — e.g.
      // `StoreIndexRegistrar` inside `StateModule.forFeature(...)`
      // requesting `StateRegistry` (provided by
      // `StateModule.forRoot()`).
      if (
        (dynamicMetadata?.global ||
          this.isGlobalModule(type, dynamicMetadata)) &&
        !existingModule.isGlobal
      ) {
        existingModule.isGlobal = true;
        this.globalModules.add(existingModule);
      }
   ...
   */
  public addExport(
    toExport: InjectionToken | IProvider | DynamicModule,
    token: string,
  ): void {
    const moduleRef = this.modules.get(token);
    if (!moduleRef) return;
