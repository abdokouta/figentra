...
import { Optional } from "../../src/core/decorators/optional.decorator";
import { Module } from "../../src/core/decorators/module.decorator";
import { Global } from "../../src/core/decorators/global.decorator";
import { SetMetadata } from "../../src/core/decorators/set-metadata.decorator";
import {
...
describe("@Injectable()", () => {
  ...
  it("stores scope options as metadata when provided", () => {
    ...
    const options = getMetadata<{ scope: Scope }>(
      SCOPE_OPTIONS_METADATA,
      TransientService,
    );
...
describe("@Inject()", () => {
  ...
  it("stores property injection metadata", () => {
    ...
    expect(props).toEqual(
      expect.arrayContaining([{ key: "myProp", type: TOKEN }]),
    );
  });
  ...
  it("resolves forward references for constructor parameters", () => {
    ...
    expect(deps).toEqual(
      expect.arrayContaining([{ index: 0, param: LazyRef }]),
    );
  });
});
...
describe("@Optional()", () => {
  it("marks a constructor parameter as optional", () => {
    ...
    const optionals = getMetadata<number[]>(
      OPTIONAL_DEPS_METADATA,
      TestService,
    );
...
describe("SetMetadata()", () => {
  ...
  it("attaches metadata to a method", () => {
    ...
    const cache = getMetadata<{ ttl: number }>(
      "cache",
      instance.getUsers as any,
    );
