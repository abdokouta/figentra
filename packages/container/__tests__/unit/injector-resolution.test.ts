...
import { Injector } from "../../src/core/container/injector.service";
import { Module } from "../../src/core/container/module";
...
describe("Injector Resolution", () => {
  ...
  describe("imports resolution", () => {
    ...
    it("does not resolve from imported module if token is not exported", async () => {
      ...
      await expect(
        injector.resolveProviders(consumerModuleRef),
      ).rejects.toThrow(/Cannot resolve dependency/);
    });
  });
