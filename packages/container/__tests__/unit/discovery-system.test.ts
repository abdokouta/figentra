...
import { ApplicationFactory } from "../../src/core/application/application.factory";
import { clearGlobalApplicationContext } from "../../src/core/utils/global-application.util";
import { Injectable } from "../../src/core/decorators/injectable.decorator";
import { Module } from "../../src/core/decorators/module.decorator";
import { DiscoveryService } from "../../src/core/discovery/discovery.service";
import { DiscoveryModule } from "../../src/core/discovery/discovery.module";
import { DiscoverableMetaHostCollection } from "../../src/core/container/discoverable-meta-host-collection.registry";
...
describe("Discovery System", () => {
  ...
  describe("createDecorator", () => {
    ...
    it("registers class with DiscoverableMetaHostCollection when applied", () => {
      ...
      expect(
        DiscoverableMetaHostCollection.metaHostLinks.has(TaggedService),
      ).toBe(true);
      expect(
        DiscoverableMetaHostCollection.metaHostLinks.get(TaggedService),
      ).toBe(MyDecorator.KEY);
    });
    ...
    it("works as a method decorator without registering in metaHostLinks", () => {
      ...
      expect(
        DiscoverableMetaHostCollection.metaHostLinks.has(TestService),
      ).toBe(false);
    });
  });
  ...
  describe("getProviders", () => {
    ...
    it("filters providers by metadataKey", async () => {
      ...
      @Module({
        ...
        providers: [TaggedService, UntaggedService],
      })
      class AppModule {}
  ...
  describe("getMetadataByDecorator", () => {
    ...
    it("returns undefined for non-decorated method", async () => {
      ...
      const wrappers = discovery.getProviders({ metadataKey: ClassDec.KEY });
      const meta = discovery.getMetadataByDecorator(
        Handler as any,
        wrappers[0],
        "undecorated",
      );
