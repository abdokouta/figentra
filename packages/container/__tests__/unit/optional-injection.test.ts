  ...
  describe("Constructor parameter optional injection", () => {
    it("should inject undefined for missing optional dependency", async () => {
      ...
      class ServiceWithOptional {
        constructor(
          @Optional() @Inject(MISSING_TOKEN) public optionalDep?: any,
        ) {}
      }
    ...
    it("should inject the value when optional dependency is available", async () => {
      ...
      class ServiceWithOptional {
        constructor(
          @Optional() @Inject(OPTIONAL_TOKEN) public optionalDep?: string,
        ) {}
      }

      @Module({
        providers: [
          { provide: OPTIONAL_TOKEN, useValue: "present" },
          ServiceWithOptional,
        ],
      })
      class TestModule {}
    ...
    it("should handle mix of required and optional dependencies", async () => {
      ...
      @Module({
        providers: [
          { provide: REQUIRED_TOKEN, useValue: "required-value" },
          MixedService,
        ],
      })
      class TestModule {}
