...
Cloudflare's current Terraform documentation recommends `cloudflare_ruleset`
for rate limiting and WAF rules. The module deliberately leaves the managed
WAF ruleset ID as an input because Cloudflare plan/account configuration can
change which managed ruleset is available.
